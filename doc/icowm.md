...
and restricted-memory mode's run-time behavior.  For the JSON
...
The normal quit action (the "Quit" keybinding, its confirmation dialog,
and the root menu's "Quit" entry) asks every currently open application
...
- **Icon pictures and hint characters.** A minimized window still shows
  ...
  without the application's picture drawn on it or any overlay character
...
- **Modern font rendering.** Every font the active theme specifies is
  replaced with IcoWM's fixed built-in font, unless it already names
...
- **XSettings propagation.** Theme and font settings are not published
  ...
  active theme's `xsettings.is-enabled`.
- **Systray icon embedding.** The systray itself (clock, battery, and
  its frame) still shows when enabled, but never accepts a docked
  application icon.
...
option skip straight to that desktop's windows instead of first asking
you to pick a desktop you do not have a choice about anyway; this is the
same behavior an ordinary session gets with only one desktop configured,
restricted-memory mode or not.
...
Once running, IcoWM keeps an eye on its memory use, checking every few
seconds.  If it ever reaches the ceiling you set, a dialog appears
...
carries its real, ongoing cost regardless of anything else.  This limit
is worked out from the ceiling you chose with `-M <mib>`, roughly like
this:
...
than that, down to 24 windows at `-M <mib>`'s smallest accepted value
(14 MiB).  Once you are at that limit, opening another application shows
a warning dialog explaining that a window has to be closed first; the
new window's application is left waiting rather than being handed
something broken to work with.
...
any other fields are that command's arguments.  A response is always
a JSON object with at least a boolean `"ok"` field: `true`, with that
command's result fields alongside it, on success; `false`, with a string
`"error"` field explaining what went wrong, on failure.  A malformed
request (not valid JSON, valid JSON with no `"cmd"`, an unrecognized
...
- Adding or removing a client from a desktop's internal tracking (as
  ...
  normally paired with, could leave IcoWM's internal state inconsistent
...
its needed, as soon as it loses focus.  While it holds focus and is
fullscreen, `set_layer_above_client`, `set_layer_normal_client`,
`set_layer_below_client`, and `cycle_layer_client` are all silently
no-ops for it, and the window context menu's "Layer" submenu is disabled
the same way, for the same reason: choosing a layer here would have no
visible effect until it later leaves fullscreen, which would read as
broken rather than merely deferred.
