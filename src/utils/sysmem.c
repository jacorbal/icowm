 ...
 *
 * Both @p sysmem_available_mib and @p sysmem_self_rss_mib need exactly
 * this same read-a-file, scan-its-lines-for-one-label,
 * parse-the-number-after-it sequence; this is the part that was
 * actually identical between the two, parameterized by which file and
 * which label each one cares about.
 *
        ...
        const char *label, uint32_t *out_mib)
{
    ...
    while (fgets(buf, (int) sizeof(buf), f) != NULL) {
        ...
        if (safe_strncmp(buf, label, label_len) == 0 &&
