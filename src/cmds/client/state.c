        ...
        struct position_s content, int32_t bw, int32_t th)
{
    ...
    /* The titlebar and content are always mapped inside the frame, but
     * the frame itself only while the client is on screen: one on a
     * desktop not shown, whose window was unmapped and so produced no
     * 'UnmapNotify' above to absorb, gets its frame mapped once that
     * desktop is shown, like any other client there */
    if (ccmd_client_is_on_screen(client)) {
...
static void s_ccmd_decorate_remove(client_td *client, int32_t bw)
{
    if (client->frame != 0) {
        ...
        /* Mapped inside the frame, the window stays mapped once on the
         * root, where nothing keeps it out of sight any more; one whose
         * desktop is not shown is unmapped again until that desktop
         * is, its own 'UnmapNotify' events absorbed the same way */
        if (!ccmd_client_is_on_screen(client)) {
            client->ignore.unmap += 2u;
            client->ignore.focus_unmap++;
