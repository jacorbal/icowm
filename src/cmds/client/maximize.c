 ...
 * see that file's history for why.  Covers the per-axis maximize state
 * machine (horizontal, vertical, or both), and re-filling an
 ...
 *         resizable, not fullscreen, and any prior iconified or shaded
 *         state has already been cleared); @c false if @p client is
 *         @c NULL or the maximize should be refused outright
 *
 ...
 *
 * The shared implementation behind @c ccmd_client_maximize,
 * @c ccmd_client_maximize_horz, and @c ccmd_client_maximize_vert, each
 * now a thin wrapper passing its own fixed @p dir; matches Openbox's
 * own @c client_maximize (@c client.c), which takes the identical
 * @p dir convention for the identical reason: one function, one place
 * the demote/complete/fresh-maximize decision is made, rather than the
 ...
 * @c CLIENT_STATE_MAXIMIZED, @c CLIENT_STATE_MAXIMIZED_HORZ, or
 * @c CLIENT_STATE__MAXIMIZED_VERT), so "is the horizontal axis
 * currently maximized" is derived
 * (@c state @c == @c MAXIMIZED @c || @c state @c == @c MAXIMIZED_HORZ)
 * rather than read directly off a field of its own; @p dir @c == @c 0
 * (both axes) still only ever has the two cases Openbox's own top-level
 * toggle does (already fully maximized, so restore; anything else, so
 ...
 * maximized (restoring from @c layout.geometry.old, keeping the other
 * axis exactly as it is), complete to full maximize if the other axis
 * is the one currently maximized (folding this axis in from the
 * workarea without disturbing the other), or maximize this axis alone
 * fresh otherwise (saving the pre-maximize geometry first, unless some
...
/* Re-fill an already-maximized client's own geometry against its current
 * workarea */
...
/* Demote a single axis's maximize state alone, without touching geometry at
 * all */
