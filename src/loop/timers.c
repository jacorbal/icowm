 ...
 * @param candidate_ms    A countdown's remaining time, or a negative
 *                        value when that countdown is not currently
 *                        active at all
 *
...
int loop_timers_timeout(const loop_ctx_td *ctx)
{
     ...
     * that report a remaining time even while closed, so both are asked
     * only while actually open */
    if (popup_is_open()) {
    ...
    /* Shorter still while a resize-cursor poll target is being tracked
     * (see 'mouse_hover_poll_tick' in 'input/mouse/hover.h'), so an
     * undecorated client's cursor gets re-evaluated promptly as the
     ...
     * 'menu/dialog/confirm.h'): a pending click-triggered close/accept,
     * or a countdown timeout that needs its visible number to advance
    ...
    /* Shorter still while a window drag is holding the pointer against
     * a warp-eligible screen edge (see 'drag_warp_tick' in
     * 'input/mouse/drag.h'), so it still switches desktops once its
     * countdown elapses even with no further 'MotionNotify' arriving to
     * drive it. */
    s_loop_timers_tighten(&poll_timeout_ms, drag_warp_ms_remaining());

    /* Shorter still while a window drag is holding the pointer against
     * a pan-eligible screen edge (see 'drag_pan_tick' in
     ...
     * 'mouse_viewport_edge_tick' in 'input/mouse/viewport/edge.h'), so
     ...
     * where it goes (see 'place_manual_tick' in
     * 'policy/placement/manual.h'): a question nobody answers has to
     * stop being asked on time, since the pointer and the keyboard are
     * both held for as long as it stands open. */
    s_loop_timers_tighten(&poll_timeout_ms,
...
void loop_timers_tick(const loop_ctx_td *ctx)
{
    ...
    /* The memory guard runs against one stage only, and only when the
     * window manager was started with a cap at all */
