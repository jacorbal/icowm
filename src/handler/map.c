 ...
 */
static void s_map_finish(const wm_td *wm, stage_td *stage,
 ...
 * that declines to manage the window (an unresolvable stage or
        ...
        xcb_map_request_event_t *event)
{
    ...
    client = lookup_find_client(stages, event->window, NULL, NULL);
    if (client != NULL) {
    ...
    stage = lookup_stage_for_root(stages, event->parent);
    if (stage == NULL && !list_is_empty(stages)) {
        stage = (stage_td *) list_data(list_head(stages));
    }
    if (stage == NULL) {
        LOGGER_ERROR("No stage found for 'MAP_REQUEST' on root %#x",
                event->parent);
        return;
    }
    ...
    if (desktop == NULL) {
        LOGGER_ERROR("No current desktop on stage %u; mapping without"
                " management", stage->id);
    ...
    if (cctl_sn_desktop_for_window(connection, event->window,
                &origin_desktop_id)) {
        origin_desktop = stage_desktop_get(stage, origin_desktop_id);
     ...
     * full reasoning. */
    (void) cctl_sn_complete_for_pid(connection, stages, event->window);
    ...
    if (max_clients > 0u &&
        ...
        memguard_warn_client_cap(connection, stage,
                config);
        return;
    }
     ...
     * placement */
    if (rules_apply(wm, client, &stage, &desktop, RULES_TRIGGER_MAP)) {
        wm_outdate_stage(stage);
        wm_outdate_desktop(desktop);
    }
    ...
    if (client->properties.type != (uint16_t) CLIENT_TYPE_DOCK &&
        ...
        if (!client->hints_icccm.hints.is_initial_iconic &&
                place_manual_enqueue(connection, wm, stage, desktop,
    ...
    s_map_finish(wm, stage, desktop, client);
}
        ...
        list_td *stages, xcb_unmap_notify_event_t *event)
{
    ...
    if (client != NULL) {
        ...
        /* 'wm_outdate_client'/'_stage'/'_desktop' directly, not
         * 'wm_request_client_redraw': 'stage' and 'desktop' are
         ...
         * 'wm->stages' for 'stage' alone, for values already
         * sitting in scope. */
        ...
        list_td *stages, xcb_circulate_notify_event_t *event)
{
    ...
    /* The stacking order changed; mark the stage so 'wm_ewmh_sync'
     * updates '_NET_CLIENT_LIST_STACKING' on the next iteration */
    stage = lookup_stage_for_root(stages, event->event);
    wm_outdate_stage(stage);
}
