/**
 * @file stage/desktops.c
 *
 * @brief Desktop-list membership and grid navigation for a stage
 *
 ...
 * a name tied to whichever axis a one-row (or one-column) stage
 ...
 * every stage that never configures a layout at all (the common case,
 ...
 * @a s_stage_desktop_direction below, can pass a tentative, possibly
 ...
 * allowance.
 *
 * @param stage      Stage whose configured layout to navigate
 ...
 *
 * @return The neighboring desktop, or @c NULL if @p stage / its
 ...
 *       @c rows @c * @c columns
 */
static desktop_td *s_stage_desktop_direction(stage_td *stage,
        ...
        bool cycle)
{
    ...
    for (uint32_t step = 0u; step < total_cells; ++step) {
        ...
        if (s_layout_index(row, col, layout, &candidate) &&
                candidate < stage->desktop_count) {
            return stage_desktop_get(stage, candidate);
        }
    }
...
/**
 * @brief Get a desktop's row/column position in its stage's
 *        configured layout
 *
 ...
 *                   @c true return
 *
 * @return @c false if @p stage, its configuration, or either
 ...
 */
static bool stage_desktop_row_col(const stage_td *stage,
        ...
        bool shows_name, char *out_label, size_t length)
{
     ...
     * own; see 'stage_desktop_row_col' above for what "row 0" always
     * means on a linear (or unconfigured) layout, the exact case this
     * excludes. */
    has_row_col = stage_desktop_row_col(stage, desktop_id,
            &row, &col);
    shows_row_col = has_row_col && stage->config != NULL &&
        stage->id < (uint32_t) CONFIG_MAX_SCREENS &&
...
int stage_desktop_add(stage_td *stage, desktop_td *desktop)
{
    ...
    if (cdlist_ins_next(stage->desktops,
                cdlist_tail(stage->desktops), desktop) != 0) {
...
int stage_desktop_rem(stage_td *stage, uint32_t desktop_id)
{
    ...
    current_item = cdlist_head(stage->desktops);
    for (size_t i = 0;
            i < stage->desktop_count && current_item != NULL;
            ++i) {
        ...
        if (desktop->id == desktop_id) {
            ...
            /* Remove the desktop */
            if (cdlist_rem_next(stage->desktops,
...
{
    return s_stage_desktop_direction(stage, desktop_id,
            S_GRID_NORTH, cycle);
}
...
int stage_desktop_select_west(stage_td *stage, bool cycle)
{
    ...
    if (stage == NULL || stage->desktop_count == 0) {
        return -1;
    }

    west_desktop =
        stage_desktop_west(stage, stage->desktop_cur, cycle);
    if (west_desktop) {
        /* Update ID of new current desktop */
        stage->desktop_cur = west_desktop->id;
        return 0;
    }
...
int stage_desktop_select_east(stage_td *stage, bool cycle)
{
    ...
    east_desktop =
        stage_desktop_east(stage, stage->desktop_cur, cycle);
    if (east_desktop) {
        /* Update ID of new current desktop */
        stage->desktop_cur = east_desktop->id;
        return 0;
    }
...
int stage_desktop_select_north(stage_td *stage, bool cycle)
{
    ...
    north_desktop =
        stage_desktop_north(stage, stage->desktop_cur, cycle);
    if (north_desktop) {
        /* Update ID of new current desktop */
        stage->desktop_cur = north_desktop->id;
        return 0;
    }
...
int stage_desktop_select_south(stage_td *stage, bool cycle)
{
    ...
    south_desktop =
        stage_desktop_south(stage, stage->desktop_cur, cycle);
    if (south_desktop) {
        /* Update ID of new current desktop */
        stage->desktop_cur = south_desktop->id;
        return 0;
    }
...
int stage_desktop_select(stage_td *stage, uint32_t desktop_id)
{
    ...
    current_item = cdlist_head(stage->desktops);
    for (size_t i = 0; i < stage->desktop_count; ++i) {
        const desktop_td *desktop =
            (desktop_td *) cdlist_data(current_item);
        if (desktop->id == desktop_id) {
            /* Update ID of new current desktop */
            stage->desktop_cur = desktop_id;
            return 0;
        }
        current_item = cdlist_next(current_item);
    }
...
/* Visit every desktop a stage holds, in order */
...
/* How many desktops a stage holds */
uint32_t stage_desktop_count(const stage_td *stage)
{
    if (stage == NULL || stage->desktops == NULL) {
        return 0u;
    }

    return (uint32_t) cdlist_size(stage->desktops);
}
