/**
 * @file stage/actions/client.c
 *
 * @brief Client show/hide, pinned transfer, and reflow for a stage
 *
 * One of the files @c stage/actions/ is made of; everything here
 * operates on a desktop's clients directly (as opposed to
 * @c stage/actions/randr.c's RandR output/CRTC/mode concerns, which
...
struct s_reflow_ctx_s {
    stage_td *stage;    /**< Stage whose monitors are consulted */
 ...
 * @param data   The stage, as a @c stage_td pointer
 *
 ...
 *
 * Shared by @a stage_client_pinned_transfer_all's counting pass
...
static void s_client_reflow_visit(client_td *client, void *data)
{
    ...
    stage_td *const stage =
        (reflow_ctx != NULL) ? reflow_ctx->stage : NULL;
    ...
    if (client != NULL) {
        ...
        for (uint32_t mi = 0u; mi < stage->monitor_count; ++mi) {
            const monitor_td *m = &stage->monitors[mi];
        ...
        if (!still_on_a_monitor) {
            ...
            int32_t margin = (int32_t)
                ((stage->config->base.windows.move_step > 0u)
                 ? stage->config->base.windows.move_step
                 : 1u);
            int32_t new_x = cx;
            int32_t new_y = cy;
            uint16_t mask;
...
void stage_client_hide_all(stage_td *stage, uint32_t desktop_id)
{
    ...
    stacking_walk(desktop, s_client_hide_visit, stage);
}
...
void stage_client_show_all(stage_td *stage, uint32_t desktop_id)
{
    ...
    stacking_walk(desktop, s_client_show_visit, stage);
        ...
        uint32_t to_id)
{
    ...
    to_desktop = stage_desktop_get(stage, to_id);
    if (to_desktop == NULL) {
        return;
    }
        ...
        if (from_desktop != NULL && from_desktop != to_desktop &&
            ...
            if (pinned != NULL) {
                ...
                for (int i = n - 1; i >= 0; --i) {
                     ...
                     * 'stage_client_show_all' restores input focus to
                     * it.
                     *
