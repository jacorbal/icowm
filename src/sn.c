...
/* Clear every bit of this module's own static state, for test
 * harnesses only; see this function's own comment in 'sn.h' */
void sn_test_reset(void)
{
    ...
    s_id_counter = 0u;
    s_timeout_seconds = SN_TIMEOUT_SECONDS;
    s_cursor_busy = false;
...
/* Directly register a pending sequence with a caller-controlled
 * start time, for test harnesses only; see this function's own
 * comment in 'sn.h' */
bool sn_test_add_pending(const char *id, uint32_t started_ms_ago)
{
    ...
    if (id == NULL || s_pending_count >= SN_MAX_PENDING) {
        return false;
    }
    ...
    started_at.tv_sec -= (time_t) (started_ms_ago / 1000u);
    nsec_diff = started_at.tv_nsec -
        (long) (started_ms_ago % 1000u) * 1000000L;
    if (nsec_diff < 0) {
        started_at.tv_sec -= 1;
        nsec_diff += 1000000000L;
    }
    started_at.tv_nsec = nsec_diff;

    (void) safe_strncpy(s_pending[s_pending_count].id, id,
            sizeof(s_pending[s_pending_count].id) - 1u);
    s_pending[s_pending_count].id[
        sizeof(s_pending[s_pending_count].id) - 1u] = '\0';
    s_pending[s_pending_count].started_at = started_at;
    ++s_pending_count;
...
/* Set the two atoms 'sn_handle_client_message' matches incoming
 * events against, for test harnesses only; see this function's own
 * comment in 'sn.h' */
