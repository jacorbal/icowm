...
int desktop_render_clients(desktop_td *desktop, bool is_current)
{
    ...
    do {
        ...
        inner_h = (client->layout.geometry.cur.dim.h > top + bottom)
            ...
            /* Undecorated client: target IS the content window.
             * The 'xcb_configure_window' above delivers a real
             * 'ConfigureNotify' with screen-relative coordinates, so no
             * inner-window offset correction is needed.  However some
             * applications (e.g., gVim) only repaint on an Expose event
             * or a synthetic 'ConfigureNotify', not on a real one
             * generated by the WM's own configure call.  Send
             * a synthetic 'ConfigureNotify' (ICCCM §4.2.3) so the
             * client always knows its definitive geometry, and request
             * a redraw with 'exposures=1' so any newly exposed region
             * is filled immediately — both after initial mapping and
             * after every resize, eliminating stale content
             * fragments. */
            client_send_synthetic_configure_notify(desktop->connection,
                    client);
