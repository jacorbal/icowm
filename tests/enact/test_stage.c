/**
 * @file tests/enact/test_stage.c
 *
 * @brief Test battery for stage-level enact actions
 *        (enact/stage.c)
 *
 * Every 'scmd_stage_desktop_switch*' function (cmds/stage.c,
 * already covered on its own by
 * tests/cmds/test_stage_desktop_switch.c) is a call-counting
 * stand-in here, so this file isolates 'enact/stage.c's own
 ...
 * internal desktop-switch logic a second time.  'stage_action_
 * desktop_add', 'stage_action_desktop_remove', and 'stage_action_
 * toggle_strutless_maximize' (stage.c) are test-controlled
 ...
 * 'wm_get_config', 'wm_get_stages', and 'keyboard_load' are
 ...
 * fields cJSON actually built for it (stage_id, desktop_id), with
 ...
 * otherwise reach is stood in for.  Each 'scmd_stage_viewport_pan_
 * north/south/east/west' (cmds/stage.c) is a call-counting
 * stand-in too, letting this file isolate 'enact/stage.c's own
...
/** Call counters for each of the five 'scmd_stage_desktop_switch*'
...
/** Call-counting stand-in for @a scmd_stage_desktop_switch (cmds/
 *  stage.c)
...
/** Call-counting stand-in for @a scmd_stage_desktop_switch_north
 *  (cmds/stage.c)
 *  @note Complexity: @e O(1)
 */
...
/** Call-counting stand-in for @a scmd_stage_desktop_switch_south
 *  (cmds/stage.c)
 *  @note Complexity: @e O(1)
 */
...
/** Call-counting stand-in for @a scmd_stage_desktop_switch_east
 *  (cmds/stage.c)
 *  @note Complexity: @e O(1)
 */
...
/** Call-counting stand-in for @a scmd_stage_desktop_switch_west
 *  (cmds/stage.c)
 *  @note Complexity: @e O(1)
 */
 ...
 *  @a scmd_stage_viewport_pan_step, shared by every one of the four
 *  'enact_stage_viewport_pan_*' wrappers */
static int s_call_pan_step;
static enum compass_direction_e s_call_pan_step_direction;

/** Call-counting stand-in for @a scmd_stage_viewport_pan_step
 *  (cmds/stage.c)
 *  @note Complexity: @e O(1)
 */
 ...
 *  a scenario can tell which of the 'enact_stage_viewport_switch_*'
...
/** Call-counting stand-in for @a scmd_stage_viewport_pan_north
 *  (cmds/stage.c)
 *  @note Complexity: @e O(1)
 */
...
/** Call-counting stand-in for @a scmd_stage_viewport_pan_south
 *  (cmds/stage.c)
 *  @note Complexity: @e O(1)
 */
...
/** Call-counting stand-in for @a scmd_stage_viewport_pan_east
 *  (cmds/stage.c)
 *  @note Complexity: @e O(1)
 */
...
/** Call-counting stand-in for @a scmd_stage_viewport_pan_west
 *  (cmds/stage.c)
 *  @note Complexity: @e O(1)
 */
...
/** Call counter and last-seen argument for @a scmd_stage_viewport_
 *  goto */
static int s_call_goto;
static uint32_t s_last_goto_page;

/** Call-recording stand-in for @a scmd_stage_viewport_goto
 *  (cmds/stage.c)
 *  @note Complexity: @e O(1)
 */
...
/** Outcome each of the three stage_action_* stand-ins below should
...
/** Test-controlled stand-in for @a stage_action_desktop_add
 *  (stage.c)
 *  @note Complexity: @e O(1)
 */
...
/** Test-controlled stand-in for @a stage_action_desktop_remove
 *  (stage.c)
 *  @note Complexity: @e O(1)
 */
 ...
 *  @a stage_action_maximize_toggle_strutless (stage.c)
 *  @note Complexity: @e O(1)
 */
...
/** Link-only stand-in for @a wm_get_stages (wm.c)
 *  @note Complexity: @e O(1)
 */
...
/** Whether, the event type, and the stage_id/desktop_id fields
...
static double s_last_broadcast_stage_id;
...
void ipc_broadcast_event(uint64_t type, cJSON *fields)
{
    ...
    if (fields != NULL) {
        stage_id_field = cJSON_GetObjectItem(fields, "stage_id");
        desktop_id_field = cJSON_GetObjectItem(fields, "desktop_id");
        s_last_broadcast_stage_id = (stage_id_field != NULL)
            ? cJSON_GetNumberValue(stage_id_field) : -1.0;
...
static void s_reset(void)
{
    ...
    s_last_broadcast_stage_id = -1.0;
    s_last_broadcast_desktop_id = -1.0;
    s_last_broadcast_had_fields = false;
}


static stage_td *s_make_stage(uint32_t id, uint32_t desktop_cur)
{
...
{
    stage_td *stage = s_make_stage(3u, 7u);

    s_reset();

    enact_stage_desktop_switch(stage, 9u);
    TAP_EQ_INT(s_call_switch, 1,
            "dispatches to scmd_stage_desktop_switch exactly once");
    ...
    TAP_EQ_INT((int) s_last_broadcast_stage_id, 3,
            "the fields object carries the stage's own id");
    TAP_EQ_INT((int) s_last_broadcast_desktop_id, 7,
            "the fields object carries the stage's current desktop"
...
{
    stage_td *stage = s_make_stage(0u, 0u);

    s_reset();

    enact_stage_desktop_switch_north(stage);
    TAP_EQ_INT(s_call_switch_north, 1,
            "north dispatches to scmd_stage_desktop_switch_north"
            " exactly once");
...
static void s_test_cyclic_south_dispatches_and_broadcasts(void)
{
    ...
    TAP_EQ_INT(s_call_switch_south, 1,
            "south dispatches to scmd_stage_desktop_switch_south"
            " exactly once");
...
static void s_test_cyclic_east_dispatches_and_broadcasts(void)
{
    ...
    TAP_EQ_INT(s_call_switch_east, 1,
            "east dispatches to scmd_stage_desktop_switch_east"
            " exactly once");
...
static void s_test_cyclic_west_dispatches_and_broadcasts(void)
{
    ...
    TAP_EQ_INT(s_call_switch_west, 1,
            "west dispatches to scmd_stage_desktop_switch_west"
            " exactly once");
    TAP_EQ_INT(s_call_switch_north + s_call_switch_south +
...
{
    stage_td *stage = s_make_stage(1u, 0u);
    ...
    TAP_EQ_INT(s_call_action_desktop_add, 1,
            "stage_action_desktop_add is called exactly once");
...
{
    stage_td *stage = s_make_stage(2u, 0u);
    int fake_keysyms_storage = 0;
    ...
    TAP_EQ_INT(s_call_action_desktop_remove, 1,
            "stage_action_desktop_remove is called exactly once");
    TAP_EQ_INT(s_call_keyboard_load, 1,
...
{
    stage_td *stage = s_make_stage(4u, 0u);
    ...
    TAP_EQ_INT(s_call_action_toggle_strutless, 1,
            "stage_action_maximize_toggle_strutless is called"
            " exactly once");
    TAP_EQ_INT(s_call_broadcast, 1,
...
static void s_test_viewport_switch_dispatches(void)
{
    ...
    TAP_EQ_INT(s_call_pan_north, 1,
            "switch north dispatches to scmd_stage_viewport_pan_"
            "north exactly once");
...
static void s_test_viewport_pan_north_dispatches(void)
{
    ...
    TAP_EQ_INT(s_call_pan_step, 1,
            "north dispatches to scmd_stage_viewport_pan_step"
            " exactly once");
...
static void s_test_viewport_pan_south_dispatches(void)
{
    ...
    TAP_EQ_INT(s_call_pan_step, 1,
            "south dispatches to scmd_stage_viewport_pan_step"
            " exactly once");
...
static void s_test_viewport_pan_east_dispatches(void)
{
    ...
    TAP_EQ_INT(s_call_pan_step, 1,
            "east dispatches to scmd_stage_viewport_pan_step"
            " exactly once");
...
static void s_test_viewport_pan_west_dispatches(void)
{
    ...
    TAP_EQ_INT(s_call_pan_step, 1,
            "west dispatches to scmd_stage_viewport_pan_step"
            " exactly once");
...
static void s_test_viewport_goto_dispatches(void)
{
    ...
    enact_stage_viewport_goto(stage, 3u);
    TAP_EQ_INT(s_call_goto, 1,
            "a page go-to dispatches to scmd_stage_viewport_goto"
            " exactly once");
