/**
 * @file tests/stage/test_viewport.c
 *
 * @brief Test battery for the configured pannable-viewport size
 *        queries (stage/viewport.c)
 *
 * Both functions under test are pure readers of
 * 'stage->config->base.screens[stage->id].viewport', so every
 * scenario here builds the stage and configuration by hand and
 * calls them directly; nothing in stage/viewport.c reaches XCB, the
...
/**
 * @brief Point a zeroed stage at a zeroed configuration, ahead of
...
static void s_test_dims_reports_configured_size(void)
{
    ...
    stage_viewport_dims(&s_stage, &columns, &rows);
...
/**
 * @brief Every unusable stage reports a 1x1 viewport rather than
 *        reading anything: a null stage, one with no configuration,
...
static void s_test_dims_falls_back(void)
{
    ...
    stage_viewport_dims(NULL, &columns, &rows);
    TAP_OK(columns == 1u && rows == 1u,
            "viewport_dims: a null stage falls back to 1x1");
    ...
    stage_viewport_dims(&s_stage, &columns, &rows);
    TAP_OK(columns == 1u && rows == 1u,
            "viewport_dims: a stage with no config falls back to"
            " 1x1");

    s_reset();
    s_config.base.screens[0].viewport.columns = 4u;
    s_config.base.screens[0].viewport.rows = 4u;
    s_stage.id = (uint32_t) CONFIG_MAX_SCREENS;
    stage_viewport_dims(&s_stage, &columns, &rows);
    TAP_OK(columns == 1u && rows == 1u,
...
static void s_test_has_room(void)
{
    ...
    TAP_OK(!stage_viewport_has_room(&s_stage),
    ...
    TAP_OK(!stage_viewport_has_room(NULL),
            "has_room: a null stage has no room, matching the 1x1"
