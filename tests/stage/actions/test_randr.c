/**
 * @file tests/stage/actions/test_randr.c
 *
 ...
 *
 * The two public entry points, 'stage_action_randr_apply_profiles'
 * and 'stage_action_randr_revert_profiles', both begin with a chain
 * of guards ('stage' null, no live connection, no screen, no
 ...
 * 'stage_monitor_refresh_all' is a link-only stand-in for the same
...
/** Link-only stand-in for @a stage_monitor_refresh_all (stage/
...
static stage_td *s_make_stage(bool randr_enabled)
{
...
static void s_free_stage(stage_td *stage)
{
    free(stage->config);
    free(stage->screen);
    free(stage);
...
/* A null stage is refused outright, on the apply path */
static void s_test_apply_null_stage_refused(void)
{
    bool result;

    s_reset();

    result = stage_action_randr_apply_profiles(NULL, true);
    TAP_OK(!result, "a null stage is refused outright by apply");
}
 ...
 * how well-formed the stage otherwise is */
static void s_test_apply_no_connection_refused(void)
{
    stage_td *stage = s_make_stage(true);
    bool result;

    s_reset();

    result = stage_action_randr_apply_profiles(stage, true);
    TAP_OK(!result,
            "no live X connection is refused by apply, even with"
            " a fully-formed, RandR-enabled stage");

    s_free_stage(stage);
}


/* A stage with no XCB screen pointer at all is refused */
static void s_test_apply_null_screen_refused(void)
{
    ...
    result = stage_action_randr_apply_profiles(stage, true);
    TAP_OK(!result, "a stage with no XCB screen is refused by apply");
...
/* A stage with no config at all is refused */
static void s_test_apply_null_config_refused(void)
{
    ...
    result = stage_action_randr_apply_profiles(stage, true);
    TAP_OK(!result, "a stage with no config at all is refused by"
            " apply");
...
{
    stage_td *stage = s_make_stage(false);
    bool result;
 ...
 * from an earlier reload is 'stage_action_randr_snapshot_begin''s
...
static void s_test_apply_non_snapshotting_call_also_refused(void)
{
    ...
    result = stage_action_randr_apply_profiles(stage, false);
    TAP_OK(!result,
...
int main(void)
{
    ...
    s_test_apply_null_stage_refused();
