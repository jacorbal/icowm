/**
 * @file tests/stage/test_desktop_add_remove.c
 *
 * @brief Test battery for the desktop-grid growing on
 *        'stage_action_desktop_add' and shrinking on
 *        'stage_action_desktop_remove' (stage/switch.c)
 *
 * 'stage_action_desktop_add'/'_remove' pull in far more of the
 ...
 * 'test_lookup.c' already stands in for 'stage_desktop_get': none
 ...
 * the shape 's_stage_layout_grow_for'/'_shrink_after' (both
 * private to stage/switch.c) leave 'desktop_layout' in afterward.
 *
 ...
 *  stays empty of clients throughout, but 's_stage_desktop_
 *  evacuate' (stage/switch.c) references it regardless */
...
/** Link-only stand-in for stage_client_hide_all (stage/actions/
...
/** Link-only stand-in for stage_client_show_all (stage/actions/
 *  clients.c); see stage_client_hide_all's own comment just above */
...
/** Link-only stand-in for stage_workarea_refresh_all (stage/
...
/* The two stand-ins below exist purely because stage/switch.c
 * compiles as a single translation unit: stage_action_toggle_
 * strutless_maximize, a third public function in that file this
 * file never calls at all, references xcb_flush; s_stage_desktop_
 * evacuate (only reachable through stage_action_desktop_remove,
 ...
 * removed, so 'stage_client_hide_all'/'_show' (stood in for above)
...
static void s_test_add_grows_rows_not_columns(void)
{
    ...
    TAP_EQ_INT(stage_action_desktop_add(stage), 0,
            "add to a full 2x3 grid succeeds");
    TAP_EQ_INT((int) stage->config->base.screens[0].desktop_layout.rows,
            3, "grows to 3 rows");
    TAP_EQ_INT(
            (int) stage->config->base.screens[0].desktop_layout.columns,
            3, "columns stays untouched at 3, not reshuffled");
    TAP_EQ_INT((int) stage->desktop_count, 7,
...
static void s_test_remove_shrinks_when_row_becomes_empty(void)
{
    ...
    stage_action_desktop_remove(stage);   /* 9 -> 8 */
    stage_action_desktop_remove(stage);   /* 8 -> 7 */
    TAP_EQ_INT(stage_action_desktop_remove(stage), 0,
    ...
    TAP_EQ_INT((int) stage->desktop_count, 6, "desktop_count is now 6");
...
static void s_test_add_then_remove_round_trip(void)
{
    ...
    stage_action_desktop_add(stage);      /* 6 -> 7, grows to 3x3 */
    stage_action_desktop_remove(stage);   /* 7 -> 6, shrinks back */
    TAP_EQ_INT((int) stage->config->base.screens[0].desktop_layout.rows,
            2, "round trip: back to 2 rows, matching the start");
    TAP_EQ_INT(
            (int) stage->config->base.screens[0].desktop_layout.columns,
            3, "round trip: columns unchanged at 3 throughout");
    TAP_EQ_INT((int) stage->desktop_count, 6,
