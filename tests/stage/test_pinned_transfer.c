/**
 * @file tests/stage/test_pinned_transfer.c
 *
 * @brief Test battery for moving every pinned client to one desktop
 *
 * Exercises 'stage_client_pinned_transfer_all' (stage/actions/
 ...
 *
 * Reached only by 'stage_client_reflow_all', which nothing here
 * calls.
 *
 ...
 *
 * Reached only by 'stage_client_hide_all', which nothing here calls.
 *
 * @note Complexity: @e O(1)
 */
 ...
 *
 * Reached only by 'stage_client_show_all', which nothing here calls.
 *
 * @note Complexity: @e O(1)
 */
...
/**
 * @brief Link-only stand-in for @a stage_monitor_for_point
 *
 * Reached only by 'stage_client_reflow_all', which nothing here calls.
 *
 * @note Complexity: @e O(1)
 */
 ...
 *
 * The real one walks 'stage->desktops' by ID; this one answers
...
static void s_test_no_drop_past_old_fixed_cap(void)
{
    ...
    cdlist_ins_next(stage.desktops, NULL, from_desktop);
    cdlist_ins_next(stage.desktops, NULL, to_desktop);
    stage.desktop_count = 2u;

    stage_client_pinned_transfer_all(&stage, 1u);
...
static void s_test_unknown_target_is_a_no_op(void)
{
    ...
    stage_client_pinned_transfer_all(&stage, 99u);
...
static void s_test_multiple_source_desktops(void)
{
    ...
    cdlist_ins_next(stage.desktops, NULL, target);
    stage.desktop_count = 3u;

    stage_client_pinned_transfer_all(&stage, 2u);
