 ...
 * desktop_*'/'enact_stage_*' function, every dialog and context-menu
...
/** Fixed desktop lookup_current_desktop and stage_desktop_get answer
 ...
 *  int per enact_client, enact_desktop, enact_stage, dialog, and
...
static int s_call_enact_stage_desktop_switch_north;
static int s_call_enact_stage_desktop_switch_south;
static int s_call_enact_stage_desktop_switch_east;
static int s_call_enact_stage_desktop_switch_west;
static int s_call_enact_stage_desktop_switch;
static int s_call_enact_stage_viewport_pan_north;
static int s_call_enact_stage_viewport_pan_south;
static int s_call_enact_stage_viewport_pan_east;
static int s_call_enact_stage_viewport_pan_west;
static int s_call_enact_stage_viewport_goto;
static uint32_t s_last_desktop_switch_id;
static uint32_t s_last_viewport_goto_page;
static int s_call_enact_stage_desktop_add;
static int s_call_enact_stage_desktop_remove;
static int s_call_enact_stage_toggle_strutless_maximize;
...
static void s_reset(void)
{
    ...
    s_call_enact_stage_desktop_switch_north = 0;
    s_call_enact_stage_desktop_switch_south = 0;
    s_call_enact_stage_desktop_switch_east = 0;
    s_call_enact_stage_desktop_switch_west = 0;
    s_call_enact_stage_desktop_switch = 0;
    s_call_enact_stage_viewport_pan_north = 0;
    s_call_enact_stage_viewport_pan_south = 0;
    s_call_enact_stage_viewport_pan_east = 0;
    s_call_enact_stage_viewport_pan_west = 0;
    s_call_enact_stage_viewport_goto = 0;
    s_last_desktop_switch_id = 0;
    s_last_viewport_goto_page = 0;
    s_call_enact_stage_desktop_add = 0;
    s_call_enact_stage_desktop_remove = 0;
    s_call_enact_stage_toggle_strutless_maximize = 0;
...
client_td *ik_get_active_client(stage_td *stage, list_td *stages,
        stage_td **cs_out, desktop_td **cd_out)
{
    (void) stage;
    (void) stages;
    if (cs_out != NULL) {
...
void ik_handle_launch(enum ik_launch_e program, stage_td *stage,
...
void ik_handle_move(enum ik_move_e direction, stage_td *stage,
...
void ik_handle_resize(enum ik_resize_e edge, stage_td *stage,
        list_td *stages, const config_td *config)
{
    (void) stage;
    (void) stages;
...
/* Stand-ins: enact.h stage/desktop-switch actions */

/** Call counters for the four whole-page viewport moves, which the
 *  'keyboard.viewport.page.*' bindings reach */
static int s_call_enact_stage_viewport_switch_north;
static int s_call_enact_stage_viewport_switch_south;
static int s_call_enact_stage_viewport_switch_east;
static int s_call_enact_stage_viewport_switch_west;

void enact_stage_viewport_switch_north(stage_td *stage)
{
    (void) stage;
    s_call_enact_stage_viewport_switch_north++;
}
...
{
    (void) stage;
    s_call_enact_stage_viewport_switch_south++;
}
...
{
    (void) stage;
    s_call_enact_stage_viewport_switch_east++;
}
...
{
    (void) stage;
    s_call_enact_stage_viewport_switch_west++;
}
...
{
    (void) stage;
    s_call_enact_stage_desktop_switch_north++;
}
...
{
    (void) stage;
    s_call_enact_stage_desktop_switch_south++;
}
...
{
    (void) stage;
    s_call_enact_stage_desktop_switch_east++;
}
...
{
    (void) stage;
    s_call_enact_stage_desktop_switch_west++;
}
...
{
    (void) stage;
    s_call_enact_stage_desktop_switch++;
...
{
    (void) stage;
    s_call_enact_stage_desktop_add++;
}
...
{
    (void) stage;
    s_call_enact_stage_desktop_remove++;
}
...
{
    (void) stage;
    s_call_enact_stage_toggle_strutless_maximize++;
}
...
{
    (void) stage;
    s_call_enact_stage_viewport_pan_north++;
}
...
{
    (void) stage;
    s_call_enact_stage_viewport_pan_south++;
}
...
{
    (void) stage;
    s_call_enact_stage_viewport_pan_east++;
}
...
{
    (void) stage;
    s_call_enact_stage_viewport_pan_west++;
}
...
{
    (void) stage;
    s_call_enact_stage_viewport_goto++;
...
void enact_desktop_cycle_clients_active(xcb_connection_t *connection,
        stage_td *stage, desktop_td *desktop, uint16_t modmask,
        const config_td *config)
{
    (void) connection;
    (void) stage;
...
void dialog_fortune_show(xcb_connection_t *connection, stage_td *stage,
        const config_td *config)
{
    (void) connection;
    (void) stage;
...
void rootmenu_show(wm_td *wm, xcb_connection_t *connection,
        stage_td *stage, struct position_s pos, const config_td *config)
{
    (void) wm;
    (void) connection;
    (void) stage;
...
void popup_show(xcb_connection_t *connection, stage_td *stage,
...
void dialog_inspect_show(xcb_connection_t *connection, stage_td *stage,
...
/* KEYBIND_DESKTOP_NORTH/SOUTH/EAST/WEST: stage-relative desktop
...
static void s_test_desktop_north_south_east_west(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_DESKTOP_NORTH, 0, 0, &stage,
            NULL, NULL);
    ik_execute_binding(s_fake_wm, KEYBIND_DESKTOP_SOUTH, 0, 0, &stage,
            NULL, NULL);
    ik_execute_binding(s_fake_wm, KEYBIND_DESKTOP_EAST, 0, 0, &stage,
            NULL, NULL);
    ik_execute_binding(s_fake_wm, KEYBIND_DESKTOP_WEST, 0, 0, &stage,
            NULL, NULL);

    TAP_EQ_INT(s_call_enact_stage_desktop_switch_north, 1,
            "KEYBIND_DESKTOP_NORTH reaches" \
            " enact_stage_desktop_switch_north exactly once");
    TAP_EQ_INT(s_call_enact_stage_desktop_switch_south, 1,
            "KEYBIND_DESKTOP_SOUTH reaches" \
            " enact_stage_desktop_switch_south exactly once");
    TAP_EQ_INT(s_call_enact_stage_desktop_switch_east, 1,
            "KEYBIND_DESKTOP_EAST reaches" \
            " enact_stage_desktop_switch_east exactly once");
    TAP_EQ_INT(s_call_enact_stage_desktop_switch_west, 1,
            "KEYBIND_DESKTOP_WEST reaches" \
            " enact_stage_desktop_switch_west exactly once");
}


static void s_test_desktop_north_null_stage_is_noop(void)
{
    s_reset();
    ...
    TAP_EQ_INT(s_call_enact_stage_desktop_switch_north, 0,
            "a null stage guards KEYBIND_DESKTOP_NORTH from" \
            " reaching enact_stage_desktop_switch_north");
}


/* KEYBIND_VIEWPORT_PAN_NORTH/SOUTH/EAST/WEST: stage-relative
...
static void s_test_viewport_pan_north_south_east_west(void)
{
    ...
    TAP_EQ_INT(s_call_enact_stage_viewport_pan_north, 1,
            "KEYBIND_VIEWPORT_PAN_NORTH reaches" \
            " enact_stage_viewport_pan_north exactly once");
    TAP_EQ_INT(s_call_enact_stage_viewport_pan_south, 1,
            "KEYBIND_VIEWPORT_PAN_SOUTH reaches" \
            " enact_stage_viewport_pan_south exactly once");
    TAP_EQ_INT(s_call_enact_stage_viewport_pan_east, 1,
            "KEYBIND_VIEWPORT_PAN_EAST reaches" \
            " enact_stage_viewport_pan_east exactly once");
    TAP_EQ_INT(s_call_enact_stage_viewport_pan_west, 1,
            "KEYBIND_VIEWPORT_PAN_WEST reaches" \
            " enact_stage_viewport_pan_west exactly once");
}


static void s_test_viewport_pan_north_null_stage_is_noop(void)
{
    s_reset();
    ...
    TAP_EQ_INT(s_call_enact_stage_viewport_pan_north, 0,
            "a null stage guards KEYBIND_VIEWPORT_PAN_NORTH from" \
            " reaching enact_stage_viewport_pan_north");
}


/* KEYBIND_VIEWPORT_GOTO_0..9: jump the stage's current desktop
 * viewport straight to one of its configured pages, matching
 * KEYBIND_DESKTOP_GOTO_0..9 exactly, key index equal to page index */
...
static void s_test_viewport_goto_n_passes_correct_page(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_VIEWPORT_GOTO_0, 0, 0, &stage,
            NULL, NULL);
    TAP_EQ_INT((int) s_last_viewport_goto_page, 0,
            "KEYBIND_VIEWPORT_GOTO_0 passes page index 0 to" \
            " enact_stage_viewport_goto");

    ik_execute_binding(s_fake_wm, KEYBIND_VIEWPORT_GOTO_5, 0, 0, &stage,
            NULL, NULL);
    TAP_EQ_INT((int) s_last_viewport_goto_page, 5,
            "KEYBIND_VIEWPORT_GOTO_5 passes page index 5 to" \
            " enact_stage_viewport_goto");

    ik_execute_binding(s_fake_wm, KEYBIND_VIEWPORT_GOTO_9, 0, 0, &stage,
            NULL, NULL);
    TAP_EQ_INT((int) s_last_viewport_goto_page, 9,
            "KEYBIND_VIEWPORT_GOTO_9 passes page index 9 to" \
            " enact_stage_viewport_goto");

    TAP_EQ_INT(s_call_enact_stage_viewport_goto, 3,
            "all three KEYBIND_VIEWPORT_GOTO_N scenarios above each" \
            " reached enact_stage_viewport_goto exactly once");
}


static void s_test_viewport_goto_0_null_stage_is_noop(void)
{
    s_reset();

    ik_execute_binding(s_fake_wm, KEYBIND_VIEWPORT_GOTO_0, 0, 0, NULL,
            NULL, NULL);

    TAP_EQ_INT(s_call_enact_stage_viewport_goto, 0,
            "a null stage guards KEYBIND_VIEWPORT_GOTO_0 from" \
            " reaching enact_stage_viewport_goto");
}
...
static void s_test_desktop_show_toggles_the_flag(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_DESKTOP_SHOW, 0, 0, &stage,
            NULL, NULL);
    ...
    TAP_OK(s_last_desktop_show_flag == true,
            ...
            " stage->is_showing_desktop as the new show state");
}


static void s_test_scratchpad_toggle_needs_a_stage(void)
{
    stage_td stage;
    ...
    TAP_EQ_INT(s_call_scratchpad_toggle, 1,
            ...
            " exactly once given a stage");

    s_reset();
    ik_execute_binding(s_fake_wm, KEYBIND_WM_SCRATCHPAD_TOGGLE, 0, 0,
            NULL, NULL, NULL);
    TAP_EQ_INT(s_call_scratchpad_toggle, 0,
            "a null stage guards KEYBIND_WM_SCRATCHPAD_TOGGLE from" \
...
static void s_test_desktop_add_remove_and_strutless(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_DESKTOP_ADD, 0, 0, &stage,
            NULL, NULL);
    ik_execute_binding(s_fake_wm, KEYBIND_DESKTOP_REMOVE, 0, 0, &stage,
            NULL, NULL);
    ik_execute_binding(s_fake_wm, KEYBIND_WM_TOGGLE_STRUTLESS_MAXIMIZE, 0,
            0, &stage, NULL, NULL);

    TAP_EQ_INT(s_call_enact_stage_desktop_add, 1,
            "KEYBIND_DESKTOP_ADD reaches enact_stage_desktop_add" \
            " exactly once");
    TAP_EQ_INT(s_call_enact_stage_desktop_remove, 1,
            "KEYBIND_DESKTOP_REMOVE reaches" \
            " enact_stage_desktop_remove exactly once");
    TAP_EQ_INT(s_call_enact_stage_toggle_strutless_maximize, 1,
            "KEYBIND_WM_TOGGLE_STRUTLESS_MAXIMIZE reaches" \
            " enact_stage_toggle_strutless_maximize exactly once");
}
...
static void s_test_fortune_needs_flag_stage_and_connection(void)
{
    stage_td stage;
    config_td cfg;
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_WM_FORTUNE, 0, 0, &stage,
            NULL, &cfg);
    TAP_EQ_INT(s_call_dialog_fortune_show, 1,
            "KEYBIND_WM_FORTUNE reaches dialog_fortune_show exactly" \
            " once when the fortune feature is enabled, a stage is" \
...
static void s_test_redraw_reload_quit_shortcuts(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_WM_REDRAW, 0, 0, &stage,
            NULL, &cfg);
    ik_execute_binding(s_fake_wm, KEYBIND_WM_RELOAD, 0, 0, &stage,
            NULL, &cfg);
    ik_execute_binding(s_fake_wm, KEYBIND_WM_QUIT, 0, 0, &stage,
            NULL, &cfg);
...
static void s_test_root_menu_center_position(void)
{
    ...
    memset(&stage, 0, sizeof(stage));
    memset(&cfg, 0, sizeof(cfg));
    stage.properties.dim.w = 800;
    stage.properties.dim.h = 600;
    xcb_connection_set(s_fake_connection);
    cfg.base.menus.root.position = CONFIG_MENU_POSITION_CENTER;
    s_lookup_desktop_result = NULL;

    ik_execute_binding(s_fake_wm, KEYBIND_WM_ROOT_MENU, 0, 0, &stage,
            NULL, &cfg);

    TAP_EQ_INT(s_call_rootmenu_show, 1,
            "KEYBIND_WM_ROOT_MENU reaches rootmenu_show exactly once" \
            " given a stage and a connection");

    xcb_connection_set(NULL);
}


static void s_test_windows_menu_needs_stage_and_connection(void)
{
    stage_td stage;
    config_td cfg;
    ...
    TAP_EQ_INT(s_call_winlist_show, 1,
            ...
            " once given a stage and a connection");
...
static void s_test_root_menu_under_mouse_falls_back_on_null_reply(void)
{
    ...
    TAP_EQ_INT(s_call_rootmenu_show, 1,
            ...
            "MOUSE with a null stage->screen skips the pointer" \
            " query entirely and still reaches rootmenu_show exactly" \
            " once, falling back to the stage center");

    xcb_connection_set(NULL);
}


static void s_test_search_windows_needs_stage_and_connection(void)
{
    stage_td stage;
    config_td cfg;
...
static void s_test_client_window_menu_needs_active_client(void)
{
    ...
    TAP_EQ_INT(s_call_wincmenu_show, 1,
            ...
            " once given a resolvable active client, a stage, and a" \
            " connection");
...
static void s_test_client_family_no_stage_is_noop(void)
{
    s_reset();
    ...
    TAP_EQ_INT(s_call_enact_client_iconify, 0,
            "s_dispatch_client_action's null-stage guard keeps" \
...
static void s_test_client_family_no_desktop_is_noop(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_ICONIFY, 0, 0, &stage,
            NULL, NULL);
...
static void s_test_client_iconify_hide_close_kill(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_ICONIFY, 0, 0, &stage,
            NULL, NULL);
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_HIDE, 0, 0, &stage,
            NULL, NULL);
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_CLOSE, 0, 0, &stage,
            NULL, NULL);
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_KILL, 0, 0, &stage,
            NULL, NULL);
...
static void s_test_client_maximize_needs_maximizable(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_MAXIMIZE, 0, 0, &stage,
            NULL, NULL);
...
static void s_test_client_center_and_move_monitor_family(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_CENTER, 0, 0, &stage,
            NULL, NULL);
...
static void s_test_client_shade_pin(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_SHADE, 0, 0, &stage,
            NULL, NULL);
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_PIN, 0, 0, &stage,
            NULL, NULL);
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_STICKY, 0, 0, &stage,
            NULL, NULL);
...
static void s_test_client_info_and_inspect(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_INFO, 0, 0, &stage,
            NULL, &cfg);
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_INSPECT, 0, 0, &stage,
            NULL, &cfg);
...
static void s_test_launch_family_passes_correct_program(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_LAUNCH_TERMINAL, 0, 0, &stage,
            NULL, NULL);
    TAP_OK(s_last_launch_program == IK_LAUNCH_TERMINAL,
            "KEYBIND_LAUNCH_TERMINAL passes IK_LAUNCH_TERMINAL to" \
            " ik_handle_launch");

    ik_execute_binding(s_fake_wm, KEYBIND_LAUNCH_LAUNCHER, 0, 0, &stage,
            NULL, NULL);
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_LAUNCH_EDITOR, 0, 0, &stage,
            NULL, NULL);
...
static void s_test_move_family_passes_correct_direction(void)
{
    ...
    ik_execute_binding(s_fake_wm, KEYBIND_CLIENT_MOVE_UP, 0, 0, &stage,
            NULL, NULL);
...
int main(void)
{
    ...
    s_test_desktop_north_null_stage_is_noop();
    s_test_viewport_pan_north_south_east_west();
    s_test_viewport_pan_north_null_stage_is_noop();
    s_test_viewport_goto_n_passes_correct_page();
    s_test_viewport_goto_0_null_stage_is_noop();
    s_test_desktop_show_toggles_the_flag();
    s_test_scratchpad_toggle_needs_a_stage();
    ...
    s_test_fortune_needs_flag_stage_and_connection();
    s_test_redraw_reload_quit_shortcuts();
    s_test_root_menu_center_position();
    s_test_windows_menu_needs_stage_and_connection();
    s_test_root_menu_under_mouse_falls_back_on_null_reply();
    s_test_search_windows_needs_stage_and_connection();
    s_test_client_window_menu_needs_active_client();

    s_test_client_family_no_stage_is_noop();
