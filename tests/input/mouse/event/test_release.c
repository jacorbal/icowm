 ...
 * stage/desktop (when both a client and a stage list are
...
static list_td *s_drag_background_end_stages;
...
static list_td *s_lookup_stages_arg;
...
static stage_td *s_drag_end_stage;
 ...
 * @note Complexity: @e O(1)
 */
void drag_background_end(xcb_connection_t *connection, list_td *stages,
        struct position_s root_pos)
{
    s_drag_background_end_calls++;
    s_drag_background_end_connection = connection;
    s_drag_background_end_stages = stages;
...
{
    s_lookup_calls++;
    s_lookup_stages_arg = stages;
    s_lookup_window_arg = window;

    if (out_stage != NULL) {
        *out_stage = s_stub_lookup_stage;
    }
    if (out_desktop != NULL) {
 ...
 * @note Complexity: @e O(1)
 */
void drag_end(xcb_connection_t *connection, stage_td *stage,
        desktop_td *desktop, struct position_s root_pos)
{
    s_drag_end_calls++;
    s_drag_end_connection = connection;
    s_drag_end_stage = stage;
...
{
    xcb_button_release_event_t event;
    list_td *const fake_stages = (list_td *) 1;
    ...
    mouse_handle_release((xcb_connection_t *) 1, fake_stages, &event,
            NULL);

    TAP_EQ_INT(s_lookup_calls, 0,
            "drag_client returns null: lookup_find_client is skipped");
    TAP_OK(s_drag_end_stage == NULL,
            "drag_end still called, with a null stage");
    TAP_OK(s_drag_end_desktop == NULL,
            "and a null desktop");
}


/* Active drag, drag_client returns non-null but stages is null:
 * lookup_find_client is skipped (nothing to search) */
static void s_test_null_stages_skips_lookup(void)
{
    ...
    TAP_EQ_INT(s_lookup_calls, 0,
            "null stages list: lookup_find_client is skipped");
}


/* Active drag, both drag_client and stages non-null:
 * lookup_find_client is called with the dragged client's window ID,
 * and its resolved stage/desktop flow through to drag_end */
static void s_test_valid_client_and_stages_looks_up_and_forwards(void)
{
    client_td dummy_client;
    stage_td dummy_stage;
    desktop_td dummy_desktop;
    xcb_button_release_event_t event;
    list_td *const fake_stages = (list_td *) 7;
    ...
    TAP_OK(s_lookup_stages_arg == fake_stages,
            "lookup_find_client receives the same stages list");
    TAP_EQ_INT((int) s_lookup_window_arg, (int) dummy_client.id,
            "lookup_find_client is searched by the dragged client's"
            " window id");
    TAP_OK(s_drag_end_stage == &dummy_stage,
            "the resolved stage reaches drag_end");
...
static void s_test_active_background_drag_ends_and_skips_client_drag(void)
{
    ...
    TAP_OK(s_drag_background_end_stages == NULL,
            "the stages list reaches drag_background_end unchanged");
...
int main(void)
{
    ...
    s_test_null_stages_skips_lookup();
    s_test_valid_client_and_stages_looks_up_and_forwards();
