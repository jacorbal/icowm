/**
 * @file tests/test_stage_lifecycle.c
 *
 * @brief Test battery for stage lifecycle: init, resize, destroy
 *        (stage.c)
 *
 * 'stage_init' is exercised against a small, test-controlled list
 ...
 * without needing one.  'desktop_init' and 'stage_desktop_add' are
 * test-controlled stand-ins too, each answering success or failure
 * however a given test asks, so every one of 'stage_init''s own
 ...
 * 'stage_monitor_refresh_all', 'xcb_connection_get', 'xcb_screen_
 * allowed_depths_iterator', 'xcb_depth_visuals_iterator', and
 * 'logger_msg' are link-only stand-ins: none of their own branches
 * are what this file means to exercise, 'stage_init''s own control
 * flow already being fully observable through 'stage_td' itself.
 * The real 'cdlist' ('adt/cdlist.c') backs 'stage->desktops',
 * so 'stage_destroy''s teardown runs through the exact same real
...
/**
 * @brief Link-only stand-in for @a stage_monitor_refresh_all
 * @note Complexity: @e O(1)
 */
 ...
 *
 * 'stage_init''s own cleanup calls this directly on a desktop it
 * just got back from 'desktop_init' once 'stage_desktop_add' below
...
/** Whether @a stage_desktop_add should accept or refuse, and after
...
/**
 * @brief Test-controlled stand-in for @a stage_desktop_add
 *
 ...
 * @note Complexity: @e O(1)
 */
int stage_desktop_add(stage_td *stage, desktop_td *desktop)
{
    (void) stage;
    (void) desktop;
...
/* A stage_id past every registered screen fails to resolve at all,
...
static void s_test_init_unknown_screen_fails(void)
{
    ...
    stage = stage_init(NULL, 5u, 0u, (config_td *) 1);

    TAP_OK(stage == NULL,
            "a stage_id past every registered screen fails");
}
...
static void s_test_init_first_desktop_fails(void)
{
    ...
    stage = stage_init(NULL, 0u, 3u, (config_td *) 1);
...
/* A screen whose second desktop's stage_desktop_add call fails
...
static void s_test_init_desktop_add_fails(void)
{
    ...
    stage = stage_init(NULL, 0u, 2u, (config_td *) 1);

    TAP_OK(stage == NULL,
            "a stage_desktop_add failure on the second desktop"
            " unwinds and returns null");
}


/* A fully successful init returns a well-formed stage with every
 * fresh-stage field at its documented default */
static void s_test_init_success_fields(void)
{
    ...
    stage = stage_init(NULL, 1u, 2u, config);

    TAP_OK(stage != NULL, "a fully successful init returns non-null");
    TAP_EQ_INT((int) stage->id, 1, "the stage's own id is set");
    TAP_OK(stage->config == config, "the config pointer is stored"
            " as given");
    TAP_OK(!stage->is_showing_desktop,
            "is_showing_desktop starts false");
    TAP_OK(!stage->strutless_maximize,
            "strutless_maximize starts false");
    TAP_OK(!stage->randr.is_known, "randr.is_known starts false");
    TAP_OK(stage->is_outdated, "is_outdated starts true");
    TAP_EQ_INT((int) stage->properties.dim.w, 1920,
            "stage width comes from the matched fake screen");
    TAP_EQ_INT((int) stage->properties.dim.h, 1080,
            "stage height comes from the matched fake screen");
...
static void s_test_init_zero_size_screen_dpi_zero(void)
{
    ...
    stage = stage_init(NULL, 0u, 1u, (config_td *) 1);

    TAP_OK(stage != NULL, "init still succeeds with a zero-size"
            " physical screen");
    TAP_EQ_INT((int) stage->properties.dpi.x, 0,
            "x DPI is zero rather than a division by zero");
    TAP_EQ_INT((int) stage->properties.dpi.y, 0,
...
static void s_test_init_zero_desktops_succeeds(void)
{
    ...
    stage = stage_init(NULL, 0u, 0u, (config_td *) 1);

    TAP_OK(stage != NULL, "zero desktops still succeeds");
    TAP_EQ_INT((int) stage->desktop_count, 0,
...
/* Destroying a null stage is a silent no-op */
static void s_test_destroy_null_is_noop(void)
{
    stage_destroy(NULL);

    TAP_OK(true, "destroying a null stage never crashes");
}


/* Resizing a stage updates its own dimensions directly, nothing
 * else */
static void s_test_resize_updates_dimensions(void)
{
    ...
    stage_resize(&stage, 1024u, 768u);

    TAP_EQ_INT((int) stage.properties.dim.w, 1024,
            "width is updated to the new value");
    TAP_EQ_INT((int) stage.properties.dim.h, 768,
