/**
 * @file tests/surface/test_sticky_transfer.c
 *
 * @brief Test battery for moving every sticky client to one desktop
 *
 * Exercises 'surface_clients_sticky_transfer_all' (surface/actions/
 * clients.c) linked against the real stacking order (policy/
 * stacking.c) and the real per-desktop client lookup (desktop/
 * dfind.c), so a sticky client is only ever seen by the function
...
static void s_test_no_drop_past_old_fixed_cap(void)
{
    ...
    surface_clients_sticky_transfer_all(&surface, 1u);
...
/* A desktop with no sticky clients at all is skipped outright: no
 * allocation, no move, no send-back */
static void s_test_no_sticky_clients_is_a_no_op(void)
{
    surface_td surface;
    desktop_td *from_desktop;
    desktop_td *to_desktop;
...
static void s_test_unknown_target_is_a_no_op(void)
{
    ...
    surface_clients_sticky_transfer_all(&surface, 99u);
...
static void s_test_multiple_source_desktops(void)
{
    ...
    surface_clients_sticky_transfer_all(&surface, 2u);
...
int main(void)
{
    ...
    s_test_no_sticky_clients_is_a_no_op();
