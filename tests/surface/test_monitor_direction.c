/**
 * @file tests/surface/test_monitor_direction.c
 *
 * @brief Test battery for real-geometry monitor direction resolution
 *        (surface/monitors.c)
 *
 * @a surface_monitor_direction calls nothing external at all: it is
 * pure arithmetic over @c surface->monitors[]/@c monitor_count, no
...
/** Link-only stand-in for atom_name (utils/xcb/atom.c): surface/
 *  monitors.c as a whole references it (from surface_refresh_
...
/**
 * @brief Build a surface_td with the given monitors populated,
 *        nothing else set
 */
static surface_td s_make_surface(const monitor_td *monitors,
        uint32_t count)
{
    surface_td surface;

    memset(&surface, 0, sizeof(surface));
    surface.monitor_count = count;
    for (uint32_t i = 0u; i < count; ++i) {
        surface.monitors[i] = monitors[i];
    }
    return surface;
}
...
static void s_test_2x2_grid(void)
{
    ...
    surface_td surface = s_make_surface(monitors, 4u);

    s_tap_monitor_at(
            surface_monitor_direction(&surface, monitors[0],
                COMPASS_EAST),
    ...
    s_tap_monitor_at(
            surface_monitor_direction(&surface, monitors[3],
                COMPASS_NORTH),
            1920, 0,
...
static void s_test_row_of_three(void)
{
    ...
    surface_td surface = s_make_surface(monitors, 3u);

    s_tap_monitor_at(
            surface_monitor_direction(&surface, monitors[1],
                COMPASS_EAST),
...
static void s_test_closest_by_center_with_mixed_sizes(void)
{
    ...
    result = surface_monitor_direction(&surface, monitors[0],
            COMPASS_EAST);
...
static void s_test_single_monitor_never_moves(void)
{
    ...
    surface_td surface = s_make_surface(monitors, 1u);
