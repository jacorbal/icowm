 ...
 * does not resolve to an X core font: fontconfig picks a matching
 * font file, FreeType2 rasterizes each glyph on demand, and xcb-render
 * (through the xcb-render-util convenience library, which builds the
 * CompositeGlyphs wire format correctly, since hand-encoding it is a
 ...
 * syntax, e.g., @c "DejaVu Sans:bold:size=10", or a plain family name)
 * and loads the matched font file with FreeType2.
 *
 * @param connection Pointer to the XCB connection
 * @param font_name  Font description string
 *
 * @return 0 on success, -1 if no font matched or the render setup
 *         failed
 *
 ...
 *
 * Frees the glyph set, FreeType face and library, and every xcb-render
 * object owned by the renderer, then resets its internal state.
 *
 * @note Complexity: @e O(1)
 */
 ...
 *
 * Glyphs for any codepoint not yet seen by this renderer are
 * rasterized and uploaded to the glyph set on demand, then cached for
 * later calls.
 *
 * @param connection Pointer to the XCB connection
 ...
 * only approximates width from a single fixed character width; a
 * proportional TrueType/OpenType font needs the real per-glyph value.
 *
 ...
 *        whichever font @c glyph_renderer_init last selected
 *
 * @return Font ascent in pixels; a small built-in default before the
 *         first successful @c glyph_renderer_init call
 *
 * @note Complexity: @e O(1)
 */
