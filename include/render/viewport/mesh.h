...
#ifndef RENDER_VIEWPORT_MESH_H
#define RENDER_VIEWPORT_MESH_H
 ...
 * luma reaches @c 128 counts as light and its mesh is darkened,
 * `C' = (C * (100 - P) / 100`;
 * a darker one gets a lightened mesh instead,
 * `C' = C + (255 - C) * P / 100`.
 *
 ...
 * cannot do.
 *
 * @param background Desktop background color the mesh is drawn over, as
 *                   @c 0x00RRGGBB
...
/**
 * @brief Where one axis' first dot sits inside the mesh tile for
 *        a given viewport origin
 *
 * The tile is @p spacing wide and the dot moves within it as the
 * viewport pans, which is what makes the mesh appear to travel with the
 * clients.  A negative @p origin is folded back into [0, @p spacing)
 ...
 *                pixels; never zero
 *
 * @return Dot offset within the tile, in interval [0, @p spacing)
 *
 ...
 * settings, the desktop background colors, or whether a mesh belongs on
 * screen at all, so that the next render rebuilds from whatever the
 * reloaded configuration now says.  Also releases the tile pixmaps
 * themselves, so a mesh switched off in the reloaded configuration
 ...
 *        away from
 *
 * @a viewport_mesh_cache_invalidate only sets a tile aside, because the
 * root's own @c XCB_CW_BACK_PIXMAP may still name it.  Whatever points
 * that attribute elsewhere, by installing a new tile or by setting
 * @c XCB_BACK_PIXMAP_NONE, calls this straight afterwards.
 *
 * @param connection Connection the pixmaps live on
 *
 * @note A no-op with nothing set aside, so it is safe to call after any
 *       background change rather than only after a mesh one
