 ...
 * them.  @a ri_render_client_icon is called from @c render/desktop.c's
 * own normal repaint path and from @c menu/cycle.c (repainting an
 * icon's real desktop window the moment its cycle-selection state
 * changes, rather than leaving it visually stuck until some unrelated
 * full desktop repaint happens to run).  Both are public rather than
 ...
 * sticky/pinned (@c CLIENT_FLAG_PIN), drawn in the same foreground
 * the state letter opposite it uses (@c icon.active.color.foreground
 * or @c icon.inactive.color.foreground, whichever the icon is
 * currently wearing): it is a state hint like the letters, only
