 ...
 * than the client owning the window itself) request a direct, one-shot
 * geometry change, the same request @c ccmd_client_move and
 * @c ccmd_client_resize apply for icowm's internal callers, but driven
 * by an external @c ClientMessage instead.
 *
 * @c event's @c data32[0] carries a bitmask of which of
 * @c data32[1..4] (@c x, @c y, @c width, @c height, in that order) are
 * actually present in this particular request; an axis whose flag bit
 * is unset is left exactly as it already was.  @p width and @p height,
 * per the EWMH specification, describe the client's content size, so
 * each is padded out by the matching pair of frame extents before being
 * applied to @p client's frame, when decorated.  @p y is floored at
 * @c 0 and @p width/@p height at @c WM_MIN_WINDOW_DIMENSION, the same
 * floors @c ccmd_client_move and @c ccmd_client_resize themselves
 * already enforce.
 *
 * An iconified @p client is left iconified.  Unlike
 * @c ccmd_client_shade, @c ccmd_client_fullscreen, and
 * @c ccmd_client_maximize (whose request is itself a visible state
 * change the user is asking for), silently un-iconifying a window
 * a user deliberately minimized just because an external pager sent it
 * a geometry hint would be a surprising side effect of a request that,
 * on any other client, has no visible effect at all.
 *
