/**
 * @file stage/viewport.h
 *
 * @brief Viewport size and panning-room queries for a stage
 *
 ...
 *
 * @defgroup stage_viewport Stage viewport queries
 * @ingroup stage
 */
/*
...
#ifndef STAGE_VIEWPORT_H
#define STAGE_VIEWPORT_H
...
/**
 * @brief Read the configured viewport size for @p stage's screen
 *
 * A null @p stage, one with no @c config, or one whose @c id sits
 ...
 *
 * @param stage Stage to read the viewport size for; may be
 *                    @c NULL
 ...
 *                    screens, is written; never null
 *
 * @note Implemented in @c stage/viewport.c
...
/**
 * @brief Whether @p stage's configured viewport spans more than a
 ...
 *
 * @param stage Stage whose configured viewport size to check; may
 *                be @c NULL
 *
 * @retval  true if some drag or keybind could still pan @p stage's
 *               viewport by at least one screen in some direction
 * @retval false if it is a plain @c {1,1} desktop
 *
 * @note Unlike @a scmd_stage_viewport_pan_available, this ignores
