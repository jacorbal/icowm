/**
 * @file stage/desktop.h
 *
 * @brief Desktop management for a stage: add, remove, look up,
 ...
 *
 * @defgroup stage_desktop Stage desktop management
 * @ingroup stage
 */
/*
...
#ifndef STAGE_DESKTOP_H
#define STAGE_DESKTOP_H
 ...
 * @retval -1 Invalid stage or no desktops
 *
 * @note Complexity: @e O(d), where @e d is @p stage's desktop count
 */
 ...
 *        optionally cycling
 *
 * A no-op search (always @c NULL, cycling or not) on a stage with no
 ...
 *       @c rows @c * @c columns
 *
 * @see @c ci_config_screens_load's comment, @c config/base/desktops.c
 */
 ...
 * desktop already sits in the one and only row.  Skips past any
 * desktop-less gap cell the same way @a stage_desktop_north does.
 *
 * @param stage      Pointer to the stage structure
 * @param desktop_id ID of the current desktop
 ...
 * the west of it: list-previous on a stage with no
 ...
 * see @a s_layout_row_col's comment, @c stage/desktops.c, for the
 ...
 *
 * Searches for the desktop with the given ID and returns the one toward
 * the east of it: list-next on a stage with no
 * @c topology.screens.desktops layout configured (the common case,
 * still the default), the desktop one cell east along the configured
 * grid otherwise (see @a stage_desktop_west's doc comment for the
 ...
 * (see @a stage_desktop_north's comment).  Updates @p desktop_cur
 * only on success.
 *
 * @param stage Pointer to the stage structure
 ...
 * (see @a stage_desktop_south's comment).  Updates @p desktop_cur
 * only on success.
 *
 * @param stage Pointer to the stage structure
 * @param cycle If @c true, wraps to the top row of the same column
 ...
 * (see @a stage_desktop_west's comment for what "west" means with and
 ...
 * (see @a stage_desktop_east's comment for what "east" means with and
...
/**
 * @brief How many desktops a stage holds
 *
 * @param stage Stage to ask; may be @c NULL
 *
