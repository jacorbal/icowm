 ...
 *
 * Every place in this project that unmaps a client window-manager-side
 ...
 * also needs the content window itself unmapped separately
 * (@a ccmd_client_iconify and @a ccmd_client_hide,
 * @c cmds/client/visibility.c, are the only two such callers today)
 * still has to account for, and issue, that additional unmap on its own
 * right after calling this: two more events arrive for
 * @c client->window in that case, matching this same
 * two-events-per-window rule, and this function only knows about the
 * one @p target it was actually given.
 *
 * @param client Client being unmapped; its @c ignore.unmap is
 *               incremented here
 * @param target Window to unmap: the frame when decorated, the bare
 *               content window otherwise, as @a ccmd_target_win
 *               resolves it
 *
