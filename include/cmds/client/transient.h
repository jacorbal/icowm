 ...
 * iconify, restore, or desktop change requested on any single member of
 * a transient family to the family as a whole, the same way a user
 * would expect minimizing (or sending to another desktop) a "save
 * changes?" prompt to take its parent editor window down with it, not
 * leave the two stranded apart.
 *
 * @param client Client whose transient chain to walk; returned as-is if
 *               it is not transient for anything, or if its declared
 *               parent is not (or not yet) a managed client
 *
...
/**
 * @brief Resolve which currently-mapped sibling a client transient for
 *        its whole group (ICCCM §4.1.2.6) should be treated as attached
 *        to right now, for stacking, raising, and focus redirect
 *
 * Close kin to @c s_place_window_transient_centered's search
 * (@c policy/placement/window.c), used there for this same client's
 * initial centering, with one deliberate difference.  This one excludes
 * another client also transient for its group, so an anchor is always
 * an actual application window of the group, never another such dialog
 * (see this function's implementation comment,
 ...
 *
 * @return The first currently-mapped (non-iconified),
 *          non-group-transient sibling sharing @p client's group
 *          leader, on @p client's desktop, or @c NULL if @p client is
 *          not transient for its group, has no group leader, or no such
 ...
 *        desktop is actually being looked at right now, wherever they
 *        currently are
 *
 * An answer to a transient family split across desktops (confirmed
 * directly against its source).  A pinned parent followed to a new
 * desktop leaves its modal dialog behind, but the moment someone tries
 * to focus that parent again, the dialog is moved onto the desktop the
 * parent is being interacted with on right then, so it is right there
 * to actually receive the redirected focus.
 *
 * Called from @a focus_apply (in @c policy/focus.c) only, immediately
 * before its redirect to @a ccmd_client_focus_target, not from
 ...
 *               redirected to its top-most ancestor first, the same way
 ...
 *       own top parent's transient descendants at every depth combined
 */
void ccmd_client_bring_family(client_td *client);

/**
 * @brief Collect every other member of a transient family found on one
 ...
 * matching sibling collected into an array first, rather than acted on
 * directly while still walking the family tree, since an action on one
 * sibling (an iconify, a pin, a desktop move) can itself add, remove,
 * or otherwise touch entries in that same tree.  Factored out once here
 * instead of repeated at every call site; the caller supplies its loop
 ...
 *         caller's to @c free; @c NULL if @p desktop, @p top, or
 *         @p count_out is @c NULL, no match was found on @p desktop, or
 ...
 * shared reasoning behind collecting into a snapshot at all): every
 * family-wide action that is not itself about desktops (iconify,
 * restore, hide, unhide, pin, unpin) must find every family member
 * regardless of which desktop each one happens to be registered under,
 * not just @p top's; those two can genuinely differ when @p top is
 * pinned, since pinning a client never actually moves it between
 * desktops.
 *
 * Scoping the search to @p top's desktop alone, as the desktop- move
 * actions genuinely need to (@a enact_desktop_client_send,
 ...
 * reason), silently fails to find a transient living elsewhere.  Counts
 * every match first, allocates exactly that many slots once, then fills
 ...
 *         @c NULL, no family member was found anywhere, or the
 ...
 *
 * @param member Family member found; never null
 ...
 * every caller treats it under a condition of its own, and sometimes in
 * a different order.
 *
 ...
 * twice and allocates an array, and is what a caller needs when its
 * action moves clients between desktops and so cannot walk and mutate
 * at the same time.
 *
 * @param top Top-most ancestor of the family; may be null
 * @param fn  Action applied to each descendant
 * @param ctx Context handed to @p fn unchanged; may be null
 *
...
/**
 * @brief Link a newly managed client into its parent's transient tree,
 *        if @c transient_for names an already-managed client
 *
 * Called once, right after a newly mapped client is added to its
 * desktop, so every other transient-family function in this project can
 * walk real @c client_td* pointers (@c transient_parent going up,
 * @c transients going down) instead of re-discovering "who is transient
 * for whom" by scanning every client on every desktop and comparing
 * @c transient_for window IDs each time it matters.
 *
 * @param client Newly managed client to link
 *
 * @note A null @p client, one not transient for anything, or one whose
 *       declared parent is not (or not yet) managed, is a silent no-op
 * @note Complexity: @e O(1)
 */
void client_link_transient(client_td *client);

/**
 * @brief Unlink a client from the transient tree before it stops being
 *        managed
 *
 * Removes @p client from its parent's @c transients list in true
 * @e O(1), and orphans every one of @p client's children by clearing
 * their @c transient_parent/@c transient_node back to @c NULL, since
 * the parent they were transient for is going away.  Must be called
 * before @p client itself is actually freed, from whichever code path
 * is unmanaging it.
 *
 ...
 *
 * ICCCM §4.1.2.6 dialogs exist to demand a specific answer before their
 * parent is usable again in any meaningful sense.  Called both from
 * @a ccmd_client_focus itself (@c cmds/client/focus.c) and from
 * @a focus_apply (@c policy/focus.c).  The latter needs its copy of the
 * redirected client, resolved before it does any of its own "currently
 * active client" bookkeeping (@c desktop->client_active_id and the
 * stacking-order raise), since a callee reassigning its local copy of
 * a pointer parameter (inside @a ccmd_client_focus) can never be
 * observed by its caller.  Without this, a caller of @a focus_apply
 * targeting the parent (a plain click, sloppy focus, restoring the
 * group, and the like) leaves @c client_active_id on the parent even
 * though real X11 input focus correctly ends up on the dialog, and that
 * mismatch is what leaves keyboard shortcuts unable to find "the active
 * client" at all once the dialog later closes.
 *
 * @param client Client focus was actually requested for
 *
 * @return The deepest mapped transient descendant found, or @p client
 *         itself if it has none (or @p client is @c NULL)
 *
