...
/**
 * @brief Apply a client's geometry to its target window in a single XCB
 *        call
 *
 ...
 * variants, shade/unshade, fullscreen/unfullscreen).  Builds the
 ...
 * rather than each caller building that same array by hand.
 *
 * @param client       Client whose target window to configure
 * @param target       Window to configure; @a ccmd_target_win's result
...
/**
 * @brief Move the client to a specific monitor on its own stage
 *
 ...
 *
 * @param client        Window to move
 * @param monitor_index Zero-based index into the client's stage's
...
/**
 * @brief Move the client to the monitor north of the current one on its
 *        stage
 *
 * Resolves @p client's current monitor, then calls
 * @a stage_monitor_direction in @c stage.h to find whichever one
 * of the stage's monitors genuinely lies to the north of it, by
 ...
 * replaced did.
 *
 * Deliberately never wraps around to the southmost monitor once already
 * at the northmost, the same as the @c windows.desktop move-to-desktop
 * pair, @a enact_client_send_to_desktop_north and its three siblings in
 * @c enact.h, already does not by default, without
 * @c desktops.wrap-at-bounds enabled: unlike wrapping a definite,
 * ordered list (a desktop's circular list, or the array order this pair
 * walks where no spatial meaning is available), wrapping a genuinely
 * 2-D physical arrangement has no one obviously correct meaning to
 * begin with (does "east, wrapped" mean the westmost monitor overall,
 * or the westmost one still on the same row?), so no attempt is made to
 * invent one; a monitor list, unlike a desktop one, also has no
 * equivalent of @c wrap-at-bounds to make that choice configurable in
 * the first place.
 *
 * @param client Window to move
 *
 * @note A no-op on a stage with one monitor or none, or when no monitor
 *       lies to the north of @p client's current one
 * @note Complexity: @e O(n), where @e n is the number of stages
 */
void ccmd_client_move_to_monitor_north(client_td *client);

/**
 * @brief Move the client to the monitor south of the current one on its
 *        stage
 *
 * See @a ccmd_client_move_to_monitor_north's comment for the fuller
 * reasoning, including why this never wraps around either.
 *
 * @param client Window to move
 *
 * @note A no-op on a stage with one monitor or none, or when no monitor
 *       lies to the south of @p client's current one
 * @note Complexity: @e O(n), where @e n is the number of stages
 */
void ccmd_client_move_to_monitor_south(client_td *client);

/**
 * @brief Move the client to the monitor east of the current one on its
 *        stage
 *
 * See @a ccmd_client_move_to_monitor_north's comment for the fuller
 * reasoning, including why this never wraps around either.
 ...
 *
 * @note A no-op on a stage with one monitor or none, or when no monitor
 *       lies to the east of @p client's current one
 * @note Complexity: @e O(n), where @e n is the number of stages
 */
void ccmd_client_move_to_monitor_east(client_td *client);

/**
 * @brief Move the client to the monitor west of the current one on its
 *        stage
 *
 * See @a ccmd_client_move_to_monitor_north's comment for the fuller
 * reasoning, including why this never wraps around either.
 ...
 *
 * @note A no-op on a stage with one monitor or none, or when no monitor
 *       lies to the west of @p client's current one
 * @note Complexity: @e O(n), where @e n is the number of stages
 */
