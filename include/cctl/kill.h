 ...
 * client, stuck in some loop that never processes its own X connection
 ...
 *
 * This module tracks a client's own real OS process (its @c pid_t, read
 ...
 *
 * Meant to be called once, right after @a ccmd_client_kill's own
 *       @a xcb_kill_client, with that same client's own
 ...
 *
 * For each pending registration whose own timeout has elapsed, checks
 ...
 * loop iteration the same way every other timed subsystem's own
 * @p *_tick already is.
 *
 * @note Complexity: @e O(n), where @e n is
 *       @c WM_KILL_ESCALATE_MAX_PENDING
