 ...
 * @c systray.c and shared with every file @ *.c in @c src/systray/,
 * plus
 * the handful of functions each of those files exposes for the
 * others to call: the tray is one shared dock, not one instance per
 * file, so its state (and the functions that act directly on it
 * without going through a public @c systray_*.h entry point) cannot
 * be split into fully independent modules the way, say,
 ...
 * @note Every file @ *.c in @c src/systray/ accesses it through this
 *       declaration
 */
 ...
 * inside it, in @c s_tray.icons order.
 */
 ...
 * selection; see @c systray_protocol_acquire_selection.
 *
 ...
 * they are, just hidden.  Safe to call when the selection is not
 * currently owned.
 */
