 ...
 *
 * Meant for a client's window to be given this cursor explicitly,
 ...
 *
 * Explicit beats inherited.  Once the client's window has its
 ...
 * event (which a client that intercepts pointer motion for its
 * purposes, e.g., to track hover for its UI, can prevent from ever
 ...
 *
 * Passed as @a xcb_grab_pointer's cursor argument by @a drag_start
 ...
 * over, rather than left to whatever cursor that window's attribute
 ...
 * which edge of that axis stays fixed (see @a drag_start_directed's
