 ...
 * left-pointer cursor @a wm_startup_subscribe_root_events already sets
 * up) so that @a mouse_handle_motion_hover only ever has to look one
 * up, never create one.  Safe to call more than once; only the first
 * call actually allocates anything.
 *
 ...
 *
 * Meant for a client's window to be given this cursor explicitly, once,
 * at decoration time (see @a ci_create_decorations in
 * @c client/geom.c), rather than left to inherit whatever the frame's
 * own cursor happens to currently be set to.
 *
 * Explicit beats inherited.  Once the client's window has its cursor,
 * the X server shows it the instant the pointer crosses into that
 * window, with no window-manager-side event handling required at all,
 * unlike relying on catching every possible crossing or motion event
 * (which a client that intercepts pointer motion for its purposes,
 * e.g., to track hover for its UI, can prevent from ever reaching this
 ...
 *
 * Passed as @a xcb_grab_pointer's cursor argument by @a drag_start (see
 * @c input/mouse/drag.c) so the cursor stays the move shape for the
 * whole drag regardless of which window the pointer happens to be over,
 * rather than left to whatever cursor that window's attribute is
 * separately set to.
 *
 * @return The move cursor, or @c 0 if @a mouse_resize_cursors_init has
 *         not run yet
 *
 * @note Complexity: @e O(1)
 */
...
/**
 * @brief The border-resize cursor matching a given resize drag's
 *        axis/anchor combination
 *
 * Passed as @a xcb_grab_pointer's cursor argument by @a drag_start for
 * a resize drag, the same way @a mouse_cursor_move is for a move:
 ...
 * which edge of that axis stays fixed (see @a drag_start_directed's doc
 * comment in input/mouse/drag.h for their exact meaning), together
 ...
 *
 * @return Matching resize cursor, or the plain-pointer cursor (see
 *         @a mouse_plain_cursor) when neither @p resize_w nor
 ...
 * and @a mouse_handle_enter, since any one kind of event or poll can be
