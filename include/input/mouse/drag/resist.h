...
/**
 * @brief Recompute a maximized-on-one-axis client's resistance
 *        state for the current motion event, live
 *
 ...
 * @c s_drag.client's @c properties.state (and its EWMH atoms) to
 * match, this same instant, via @c ccmd_client_demote_axis_state /
...
/**
 * @brief Settle a maximize-locked axis's final state once a
 ...
 * @c s_drag.client's state live, on the very motion event it
 * happened on, so nothing further is needed here for that case.  Under
 * @c !solid_drag, state was never touched mid-drag at all (see
 * @c drag_resist_axis_update's comment), so it still reflects
 * whichever axis this client started this drag maximized on;
 * @c s_drag.resize_w / @c s_drag_resize_h, read here fresh at the very
 * end, say whether the resistance threshold on that same axis ended up
 * crossed by release time, demoting it to match via
 * @c ccmd_client_demote_axis_state if so.  Geometry itself must already
 * be correctly settled by the time this is called, from this same
 * drag's finalize step; this only ever updates @c properties.
 * state (and its EWMH atoms) to match, never geometry.
 *
