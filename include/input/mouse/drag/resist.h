...
/**
 * @brief Recompute a maximized-on-one-axis client's own resistance
 ...
 * @c s_drag.client's own @c properties.state (and its EWMH atoms) to
 ...
 * @c drag_resist_axis_update's own comment), so it still reflects
 ...
 * drag's own finalize step; this only ever updates @c properties.
 * state (and its own EWMH atoms) to match, never geometry.
 *
 * @param finalize_resize Whether this drag's own geometry just got
