...
#define IPC_EVENT_FOCUS_CHANGED ((uint32_t) 1u << 3)    /**< A desktop's
                                                             own active
                                                             client changed */
#define IPC_EVENT_URGENCY_SET ((uint32_t) 1u << 4)      /**< A client's
                                                             urgency hint
                                                             was set */
#define IPC_EVENT_URGENCY_CLEARED ((uint32_t) 1u << 5)  /**< A client's
                                                             urgency hint
                                                             was cleared */
#define IPC_EVENT_WINDOW_MOVED ((uint32_t) 1u << 6)     /**< A client's
                                                             own position
                                                             changed */
#define IPC_EVENT_WINDOW_RESIZED ((uint32_t) 1u << 7)   /**< A client's
                                                             own size
                                                             changed */
#define IPC_EVENT_RULE_APPLIED ((uint32_t) 1u << 8)     /**< A rule changed
                                                             one of a
                                                             client's own
                                                             properties
                                                             */
#define IPC_EVENT_PIN_SET ((uint32_t) 1u << 9)          /**< A client was
                                                             pinned */
#define IPC_EVENT_PIN_CLEARED ((uint32_t) 1u << 10)     /**< A client was
                                                             unpinned */
#define IPC_EVENT_FULLSCREEN_SET ((uint32_t) 1u << 11)  /**< A client
                                                             entered
                                                             fullscreen */
#define IPC_EVENT_FULLSCREEN_CLEARED ((uint32_t) 1u << 12) /**< A client left
                                                                fullscreen */
#define IPC_EVENT_SHADE_SET ((uint32_t) 1u << 13)       /**< A client was
                                                             shaded */
#define IPC_EVENT_SHADE_CLEARED ((uint32_t) 1u << 14)   /**< A client was
                                                             unshaded */
#define IPC_EVENT_HIDE_SET ((uint32_t) 1u << 15)        /**< A client was
                                                             hidden */
#define IPC_EVENT_HIDE_CLEARED ((uint32_t) 1u << 16)    /**< A client was
                                                             unhidden */
#define IPC_EVENT_DECORATION_SET ((uint32_t) 1u << 17)  /**< A client's own
                                                             decoration
                                                             was shown */
#define IPC_EVENT_DECORATION_CLEARED ((uint32_t) 1u << 18) /**< A client's own
                                                                decoration
                                                                was hidden */
#define IPC_EVENT_CLIENT_ICONIFIED ((uint32_t) 1u << 19)   /**< A client was
                                                                iconified */
#define IPC_EVENT_CLIENT_DEICONIFIED ((uint32_t) 1u << 20) /**< A client was
                                                                restored
                                                                from being
                                                                iconified */
#define IPC_EVENT_LAYER_CHANGED ((uint32_t) 1u << 21)   /**< A client's own
                                                             stacking layer
                                                             changed */
#define IPC_EVENT_CLIENT_DESKTOP_CHANGED ((uint32_t) 1u << 22) /**< A client
                                                                    moved to a
                                                                    different
                                                                    desktop */
#define IPC_EVENT_CLIENT_RENAMED ((uint32_t) 1u << 23)  /**< A client's own
                                                             displayed
                                                             title was
                                                             overridden */
#define IPC_EVENT_CLIENT_RECLASSED ((uint32_t) 1u << 24)  /**< A client's own
                                                               @c WM_CLASS
                                                               was overridden */
#define IPC_EVENT_CLIENT_REROLED ((uint32_t) 1u << 25)  /**< A client's own
                                                             window role was
                                                             overridden */
#define IPC_EVENT_CLIENT_ICON_CHANGED ((uint32_t) 1u << 26) /**< A client's own
                                                                 displayed
                                                                 icon was
                                                                 overridden */
#define IPC_EVENT_DESKTOP_BACKGROUND_CHANGED ((uint32_t) 1u << 27) /**< A
                                                                     desktop's
                                                                     own solid
                                                                     background
                                                                     color
                                                                     was set */
#define IPC_EVENT_DESKTOP_SHOWN ((uint32_t) 1u << 28)   /**< Every client on
                                                             a desktop was
                                                             shown at once */
#define IPC_EVENT_DESKTOP_HIDDEN ((uint32_t) 1u << 29)  /**< Every client on
                                                             a desktop was
                                                             hidden at once */
#define IPC_EVENT_CONFIG_RELOADED ((uint32_t) 1u << 30) /**< Every
                                                             ...
                                                             file was
                                                             reloaded */
#define IPC_EVENT_STACKING_CHANGED ((uint32_t) 1u << 31)  /**< A client's own
                                                               position
                                                               within its
                                                               layer's
                                                               stacking
...
/**
 * @brief Clear every connected client slot and the listening socket
 *        state, without touching any real file descriptor
 *
 ...
 * client slot behind for the next one to unexpectedly inherit.  Any
 * real file descriptor a test itself injected via
 * @a ipc_test_inject_client remains the test's own responsibility to
 * close; this never does so on a caller's behalf.
 *
 * @note Complexity: @e O(n), where @e n is @c IPC_MAX_CLIENTS
 */
void ipc_test_reset(void);

/**
 * @brief Directly occupy a client slot with an already-connected file
 *        descriptor
 *
 * For test harnesses only, never called from any production code
 * path: lets a test exercise @a ipc_handle_readable and
 * @a ipc_poll_fds against a real, connected socket (a @c socketpair
 * end works well) without going through @a ipc_init's own real work
 * (creating the runtime directory, binding a real @c AF_UNIX socket
 * at a real path) or @a s_new_client_accept's own @c accept call,
 * neither of which either function's own logic depends on.
 *
 * @param fd Already-connected, already non-blocking file descriptor
 *           to occupy a slot with
 *
 * @return The slot index @p fd was placed into, or @c -1 if every
 *         slot is already in use
 *
 * @note Complexity: @e O(n), where @e n is @c IPC_MAX_CLIENTS
 */
int ipc_test_inject_client(int fd);

/**
 * @brief Directly set the listening-socket descriptor
 *
 * For test harnesses only, never called from any production code
 * path: @a ipc_poll_fds only lists anything at all once this is set
 * to something other than @c -1 (matching a listening socket that
 * has never been brought up), which @a ipc_init alone otherwise
 * does, by really creating and binding one. A fake, arbitrary
 * positive value is enough to satisfy that check; nothing in
 * @a ipc_poll_fds itself ever reads from or writes to it.
 *
 * @param fd Value to report as the listening socket from now on
 *
 * @note Complexity: @e O(1)
 */
void ipc_test_set_listening_fd(int fd);
