 ...
 *        system-wide available memory, and this process's own
 *        resident memory usage
 *
 * Used by restricted-memory mode (@c icowm -M): @c sysmem_available_
 * mib decides whether there is enough free system memory to start at
...
/**
 * @brief Read the kernel's own estimate of currently available
 *        system memory
 *
 ...
 * new applications without swapping, already accounting for
 * reclaimable caches and buffers, so this is a meaningfully better
 * signal than @c MemFree alone would be.  Linux-specific; on any
 * other kernel, or if the file cannot be read or parsed, returns
 * @c false and leaves @p out_mib untouched.
 *
 * @param out_mib Receives the available memory, in mebibytes,
 *                rounded down
 *
...
/**
 * @brief Read the calling process's own current resident memory
 *        usage
 *
 * Reads the @c VmRSS line from @c SYSMEM_SELF_STATUS_FILE: the
 * portion of this process's own memory currently held in RAM (as
 * opposed to swapped out, or merely reserved address space never
 * actually touched), the same figure tools like @c top and @c ps
 * report.  Linux-specific; on any other kernel, or if the file cannot
 * be read or parsed, returns @c false and leaves @p out_mib untouched.
 *
 * @param out_mib Receives this process's own resident memory usage,
 *                in mebibytes, rounded down
 *
 * @return @c true on success
 *
 * @note Complexity: @e O(1), a single short file read
