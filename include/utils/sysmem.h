 ...
 * process's own usage against the configured ceiling once running.
 *
...
/**
 * @brief Read the kernel's own estimate of currently available system
 *        memory
 *
 ...
 * caches and buffers, so this is a meaningfully better signal than @c
 * MemFree alone would be.  Linux-specific; on any other kernel, or if
 * the file cannot be read or parsed, returns @c false and leaves
 * @p out_mib untouched.
 *
...
/**
 * @brief Read the calling process's own current resident memory usage
 *
 * Reads the @c VmRSS line from @c SYSMEM_SELF_STATUS_FILE.  The portion
 * of this process's own memory currently held in RAM (as opposed to
 * swapped out, or merely reserved address space never actually
 * touched), the same figure tools like @c top and @c ps report.
 *
 * @param out_mib Receives this process's own resident memory usage, in
