...
/**
 * @brief Clear every bit of this module's own static state
 *
 * For test harnesses only, never called from any production code
 * path: returns this module to what it holds right after process
 * startup (no sequence pending, no reassembly in progress, the busy
 * cursor considered not shown, both startup-notification atoms
 * un-resolved), so one test case cannot leave any of it behind for
 * the next one to unexpectedly inherit.
 *
 * @note Complexity: @e O(r), where @e r is @c SN_MAX_REASSEMBLY
 */
void sn_test_reset(void);

/**
 * @brief Directly register a pending sequence with a caller-controlled
 *        start time, bypassing every bit of @a sn_begin's own real
 *        work (generating an ID, broadcasting it, showing the busy
 *        cursor)
 *
 * For test harnesses only, never called from any production code
 * path: lets a test exercise @a sn_ms_remaining and @a sn_tick's own
 * timeout logic on demand (a sequence already past its own timeout,
 * or nowhere near it yet), without either waiting out a real timeout
 * or reaching past this module's own XCB dependencies (@a sn_begin
 * itself needs a working connection to broadcast on, this does not).
 *
 * @param id             Startup ID for the new pending sequence
 * @param started_ms_ago How many milliseconds before this call the
 *                        sequence should appear to have started;
 *                        @c 0 for "started right now"
 *
 * @return Status of the operation
 * @retval  true on success
 * @retval false if @p id is null, every pending slot is already in
 *               use, or the clock itself could not be read
 *
 * @note Complexity: @e O(1)
 */
bool sn_test_add_pending(const char *id, uint32_t started_ms_ago);

/**
 * @brief Set the two atoms @a sn_handle_client_message matches
 *        incoming events against
 *
 * For test harnesses only, never called from any production code
 * path: @a sn_begin is the only other place either atom is ever set,
 * and it does so by resolving them for real over @p connection
 * (@a atom_intern), which a test exercising @a sn_handle_client_message
 * alone, without also calling @a sn_begin, would otherwise have no
 * way to arrange.
 *
 * @param begin New value for the atom @c _NET_STARTUP_INFO_BEGIN
 *              messages carry
 * @param info  New value for the atom subsequent @c _NET_STARTUP_INFO
 *              messages carry
 *
 * @note Complexity: @e O(1)
 */
void sn_test_set_atoms(xcb_atom_t begin, xcb_atom_t info);
