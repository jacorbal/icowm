 ...
 *
 * A mouse click on a dialog's button changes which one is selected, and
 * that needs to actually be visible on screen for a moment before the
 * dialog goes away, or the click reads as though it did not register at
 * the right spot even though it did.
 *
 * Closing on the very same repaint that shows the new selection would
 * not give a user any real chance to perceive it, since screen updates
 * too fast and human perception has none of a Vulcan's freaking
 * reflexes.  Blocking with a sleep to wait one out would freeze the
 * whole window manager's event loop for that long instead.
 *
 ...
 * @a menu_dialog_defer_ms_remaining into its poll timeout first, the
 ...
 *        becomes due
 *
 * Meant to be folded into the main loop's poll timeout (see @c loop.c's
 * @c s_loop_tighten_poll_timeout), the same way every other
 * timer-driven subsystem already reports its next wake-up time.
 *
...
/**
 * @brief Run the pending deferred action once its deadline has arrived
 *
 * Meant to be called every main-loop iteration, unconditionally, the
 * same way every other timer-driven subsystem's tick function already
 * is.
 *
