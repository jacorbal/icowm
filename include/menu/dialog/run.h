 ...
 *
 * @brief Built-in run-box: a single centered text field for typing and
 *        launching a command directly, with no search, no listing, and
 *        no cache
 *
 ...
 * @c dmenu / @c gmrun), consulted only when
 * @p config_base_s.programs.use_builtin_launcher is set; see
 * @a ik_handle_launch (@c input/kbd/interact.c) for where that choice
 * is made.  Deliberately does none of what a real launcher like those
 * does (no fuzzy matching over installed programs, no history, no cache
 * of any kind).  Typing @c Return attempts to run whatever was typed
 ...
 * single centered text field, the same theme colors), by its "Run:"
 * prompt and its, narrower fixed width; see @c defs/run.h for both this
 * and the widget's dimensions.
 *
 ...
 *
 * Everything else acts at the cursor rather than at the end: the arrows
 * move it a character, @c Home and @c End take it to either edge, @c
 * Backspace removes what is behind it and @c Delete what is under it,
 * and a printable character is inserted there.  @c Ctrl+A and @c Ctrl+E
 * do what @c Home and @c End do, those being the ones a command line
 * answers to.
 *
 * @param connection XCB connection
