...
/**
 * @brief Iterate over every item in a circular doubly-linked list,
 *        head to tail
 *
 ...
 *
 * @note Complexity: @e O(n), where @e n is the list's own size
 */
#define cdlist_foreach(self, item) \
    for ((item) = cdlist_head(self); (item) != NULL; \
            (item) = (cdlist_next(item) == cdlist_head(self)) \
                ? NULL : cdlist_next(item))
