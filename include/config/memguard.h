 ...
 * own ordinary defaults, not a variant of it: restricted-memory mode
 * is meant to restrict a fixed, deliberately chosen set of behavior
 * (no aesthetic extras, one screen, one desktop, and so on) regardless
 * of what an ordinary session would otherwise default to, so the two
 * are kept as two independent functions rather than one with
 ...
 * @a config_load_memguard's own @c memguard.json parsing pass may
 * still overwrite it afterward.
 *
 * @param config Configuration structure to populate; a @c NULL @p
 *               config is a no-op
 *
 ...
 * own fixed profile first, then layers exactly four things on top of
 * it, in order: @c memguard.json (the only file restricted-memory
 * mode lets a person configure at all, covering the active theme's
 * name, launched programs, desktop margins, the systray block, and
 * the emergency shortcut), @c bindings.json (key and mouse bindings,
 * loaded the same way an ordinary session loads them), the theme file
 * @c memguard.json named (colors and decoration load normally, but
 * see below), and finally this mode's own restrictions on top of
 * whatever that theme specified.  @c config.json and @c randr.json
 * are never read at all in this mode, not even as a fallback: RandR
 * output-profile management stays disabled outright, and every
 * setting @c config.json would otherwise carry either has no
 * counterpart in @c memguard.json's own smaller schema or is one of
 * @a config_set_default_values_memguard's own fixed values instead.
 *
 ...
 * field (colors, decoration, @c is-captioned included) is left
 * exactly as the theme file specified, since a compiled-in theme
 * occupies the same memory as one read from a file and there is
 * nothing to save by skipping it.
 *
 * @param config        Configuration structure to populate
 * @param config_prefix Configuration directory prefix, or @c NULL to
 *                       resolve it the same way @a config_load does
 *
 * @return @c 0 on success; @c 1 if @c memguard.json itself could not
 *         be loaded (the fixed profile and whatever @c bindings.json
 *         and the theme file did load are still applied)
 *
