 ...
 *
 * @c move_client, @c move_client_to_monitor, and @c resize_client
 * need their own numeric arguments alongside @c "client_id", so
 * those three do not go through ipc/dispatch.h's shared wrapper;
 * the rest do, the same as ipc/actions/client/basic.h's own.
 *
 ...
 *  the new top-left corner), "w", "h" (both unsigned) */
