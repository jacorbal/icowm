 ...
 * repeating the same lookup and error wording on its own.
 *
 ...
 *
 * Reads an optional numeric @c "surface_id" field from @p args;
 * when absent, falls back to the first surface in @p wm's own list,
 * the only reasonable default on a single-monitor setup and still a
 * usable one on a multi-monitor one.
 *
 ...
 *                            response on failure (possibly @c NULL
 *                            itself, on an allocation failure
 *                            building that response); untouched on
 *                            success
 *
 * @return The resolved desktop, or @c NULL on failure (see
 *         @p out_error for why)
 *
 * @note Complexity: @e O(1)
 */
 ...
 *                    failure (possibly @c NULL itself, on an
 *                    allocation failure building that response);
 *                    untouched on success
 *
 * @return The resolved client, or @c NULL on failure (see
 *         @p out_error for why)
 *
 * @note Complexity: @e O(s * d * c), where @e s is the number of
 *       surfaces, @e d the number of desktops per surface, and @e c
 *       the hash-table lookup cost per desktop
 */
