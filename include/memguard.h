 ...
 * not here.
 *
 * @see Option @c -M in @c main.c, and @a wm_start in @c wm.h
 *
 * @ingroup loop
 */
/*
 ...
 *
 * @param ceiling_mib Restricted-memory mode's ceiling in mebibytes, or
 *                    @c 0 to turn the whole module off (both the
 ...
 *
 * @return The client cap, or @c 0 when the module is off (callers
 *         should treat @c 0 as "no cap", not as "cap of zero clients")
 *
 * @see @a memguard_init and @c MEMGUARD_KIB_PER_CLIENT
 *
 * @note Complexity: @e O(1)
 */
 ...
 * is off, when fewer than @c MEMGUARD_CHECK_INTERVAL_SECONDS have
 * passed since the last actual check, or when a warning is already
 ...
 *       reason (only one instance at a time), so a burst of several
 *       windows opening in quick succession while already at the cap
