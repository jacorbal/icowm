 ...
 *
 * Split by competency into @c startup/install.h (registering signal
 * handlers), @c startup/handle.h (the handlers themselves and the
 * flags they set, queried back by the main loop), and
 * @c startup/subscribe.h (X server event subscriptions).  This header
