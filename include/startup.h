 ...
 * @retval  0 on success
 * @retval -1 if registration fails on any surface
 *
 ...
 * reload, @c SIGINT / @c SIGQUIT / @c SIGTERM request shutdown,
 * @c SIGCONT requests input-grab restoration, and @c SIGCHLD schedules
 * zombie reaping in the main loop.
 *
 * @return Status of the operation
 * @retval  0 on success
 * @retval -1 if @a sigaction fails
 *
 ...
 * than simply dying would.
 *
 * What they do instead is make sure dying is not silent.  Each writes
 * a short diagnostic naming the signal directly to standard error using
 * nothing but the @a write syscall (the only output primitive POSIX
 * guarantees safe to call from a signal handler; the logger's own
 * buffered, allocating machinery is not), then restores that signal's
 * default disposition and re-raises it, so the process actually
 * terminates through the normal mechanism afterward (a core dump, if
 * the system is configured to produce one, and the correct exit status
 * reported to whatever started the humble yet remarkably redoubtable
 * @c icowm).
 *
 * A client application crashing on its own (for example, failing its
 * own graphics initialization) does not, by itself, send @c icowm any
 * signal at all; installing this does not change whether such a crash
 * brings @c icowm down with it; see @a xcb_connection_has_error's own
 * handling in @c loop.c for the separate, and far more likely,
 * mechanism behind that (the X server itself becoming unreachable,
 * which no window manager can recover from either, being just another
 * client of that same server) instead.
 *
 * @return Status of the operation
 * @retval  0 on success
 * @retval -1 if @a sigaction fails
