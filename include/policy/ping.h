 ...
 * unresponsive once @c WM_EWMH_PING_TIMEOUT_SECONDS worth of those
 * have gone unanswered.  @a handler_message_event (handler/message.c)
 * clears that mark again the moment any reply, even a late one,
 * finally arrives.  All scan state is private to the implementation.
 *
 * @ingroup policy
 */
/*
 ...
 * cmds/client/ewmh.h), and one still waiting on a previous probe's
 * reply has its pending-round count advanced; once that count covers
 ...
 * @retval -1 no client currently advertises @c _NET_WM_PING support
 *            (as of the most recent @a ping_tick call), so there is
 *            nothing to probe
 *
